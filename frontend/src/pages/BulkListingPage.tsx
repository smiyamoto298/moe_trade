...
/**
 * 公式サイトの所持アイテム一覧（タブ区切り）を一括出品用に解析する。
 * 解析自体は共通トークナイザ {@link parseItemBox} に委ね、ここでは出品できる転送○のみを
 * 行に採用し、転送×・空きスロットは除外件数として集計する。
 */
export function parsePaste(text: string): { rows: Row[]; excluded: number; skipped: number } {
  const { rows: parsed, emptyExcluded, emptySkipped, noCellSkipped } = parseItemBox(text)

  const rows: Row[] = []
  let excluded = emptyExcluded
  parsed.forEach((r, i) => {
    // 転送×（トレード不可）は出品できないため除外件数に数える
    if (isTransferNg(r.tenso)) {
      excluded++
      return
    }
    rows.push({
  ...
  return { rows, excluded, skipped: noCellSkipped + emptySkipped }
}
...
export default function BulkListingPage() {
  ...
  // 除外アイテム（管理者の共通＋自分の個別）をマージしたセット。貼り付け時に除外する。
  ...
  // 除外アイテムを読み込む（共通＝API、個別＝所持品台帳の保存先から）
  useEffect(() => {
    if (!user) return
    Promise.all([
  ...
  const handleLoad = () => runLoad(async () => {
    setErrors([])
    setResult(null)
    const { rows: allRows, excluded: ex } = parsePaste(raw)
    // 除外アイテム（共通＋個別）に一致する行を除外し、除外件数に加算する
    const parsedRows = allRows.filter((r) => !isExcluded(r.name, exclusionSet))
    setExcluded(ex + (allRows.length - parsedRows.length))
    setParsed(true)
              ...
              <table className="w-full min-w-[680px] text-sm">
                <thead>
                  <tr className="bg-surface text-gray-400 text-xs">
                    ...
                    <th className="px-2 py-2 text-center font-medium">染色</th>
                      ...
                      <tr key={r.key} className={overQuantity ? 'bg-red-900/30' : linked ? '' : 'bg-yellow-900/5'}>
                        ...
                        <td className="px-2 py-2 text-center">
                          <input
                            ...
                            disabled={!linked}
                            checked={r.isDyed}
                            onChange={(e) => setRow(r.key, { isDyed: e.target.checked })}
                            title="染色済み（染色液で色を変更済み）"
                            className="accent-fuchsia-500 disabled:opacity-40"
                          />
                        </td>
                        <td className="px-2 py-2 text-center">
                          <button
