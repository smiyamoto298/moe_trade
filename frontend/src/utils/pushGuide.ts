    ...
    if (isIOS) {
      return {
        ...
        message: [
          ...
          '4. アプリでログインし、「⚙️ 通知設定」の「🔔 ブラウザ通知を有効にする」をタップ',
          '5. 「通知を送信します」の確認で「許可」を選択',
          '',
          '※ Safari で設定した通知はアプリに引き継がれません。インストール後、アプリ内で通知を再度設定してください。',
        ].join('\n'),
        highlight:
          'iOS 16.4 以降で利用できます。通知が届かない場合は、iPhone の「設定」→「通知」→「MoE Trade」で通知が許可されているか確認してください。',
      }
    }
    return {
      title: 'プッシュ通知を利用するには',
