...
class ExcludedItemController extends Controller
{
     ...
     *
     * 集計は通知バッジ件数（`notifications/summary` の `excluded_suggestion_count`）と共有するため
     * {@see \App\Support\ExclusionSuggestions} にある（候補の定義もそちらのコメントを参照）。
     *
    ...
    {
        return response()->json(ExclusionSuggestions::rows());
    }
