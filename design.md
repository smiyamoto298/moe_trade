...
- **MoE公式素材の扱い（重要）**: `public/img/` のうち **公式配布バナー（`banner/`・`side_banner/`）・ファンサイトキット由来のイラスト（`castle.jpg`）・ゲーム内アイコン（`favicon.jpg`）は MoE ファンサイトガイドラインにより「個人の非営利WEBサイトでのみ使用可」のため、公開リポジトリに含めない**（`.gitignore` 済み。正本は非公開の運用置き場で管理し、ローカル・本番には実ファイルを配置する）。自作素材（`logo*.png`・`site_ogp.jpg`・`tours/`）は追跡してよい
