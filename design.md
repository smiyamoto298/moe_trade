...
- **起動**: ヘッダー「管理」メニューの「**本番データ取込**」ボタン（**ローカル環境・admin 限定で表示**、`import.meta.env.DEV`）から `POST /api/admin/dev/pull-prod`（ローカル専用・本番は 403）を呼ぶ。確認ダイアログ→実行→要約をダイアログ表示。チャット本文や掲示板投稿などの自由記述に含まれうる個人情報はマスキング対象外（構造化された本番固有情報のみマスクする）。
