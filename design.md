...
- **MoE アカウント**: 公式のアイテムボックスはアカウント単位のため、所持品整理用のアカウント名を複数登録できる（取引用キャラクター `user_characters` とは別概念）。アカウントごとに貼り付け、表示は「すべて／アカウント別」を**タブ（横並びのボタン）で切替**（各タブに件数を併記。旧データの未割り当てが残る場合のみ「未割り当て」タブも表示）。**所持アイテムは必ずいずれかのアカウントに属する（未割り当ては許容しない）**。アカウント削除時は紐づくアイテムも削除する（旧データに残る未割り当ては表示・閲覧のみ可）
...
- **表示種別（ジャンル）**: 旧「除外リスト」を廃止し、各アイテムを**種別（ジャンル）で分類して表示を切り替える**方式に変更（取り込みでは弾かず全件保持）。行の**実効種別**は `effectiveTypeId`（`frontend/src/utils/inventory.ts`）で次の優先順位で決まる: ①登録アイテムに紐づく（`itemId!=null`）→ **取引可能**（派生種別）、②**共通の種別割当**（管理者・`excluded_items` の name→`exclusion_type_id`）、③**ユーザーの種別割当**（`user_excluded_items` の name→`exclusion_type_id` / ローカル保存時はクライアント。`null` は既定種別「その他」）、④どれも無し → **未登録**（内部値 `unset`）。
  ...
  - **一覧の種別列・対象外種別ボタン**: 一覧に種別列を設け、行の実効種別をバッジ表示する。**未登録（種別未割当かつアイテム未登録）**の行には、アイテムの「新規登録」ボタンとは別に「**対象外種別**」ボタンを表示し、クリックで種別選択ダイアログを開く（**管理者は新規種別を登録可**＝`createType`、それ以外は既存種別から選択）。割当はアイテム名単位で同名の全行に効く。ユーザー割当はダイアログから変更・解除できる。
...
- **表示種別・対象外の管理（`excluded_items` ほか・admin）**: アイテムボックスの共通の表示種別（ジャンル）割当を `/admin/excluded-items` で管理（改行・カンマ区切りで一括追加・検索・削除）。共通割当リストは**種別タブ＋あいうえお順**で表示し、**チェックボックスで複数選択して一括削除**できる。アイテムボックスではこの割当でアイテムの表示種別が決まる（取引可能＞共通割当＞ユーザー割当＞未設定）。**ユーザーが分類した名前（DB保存分）を集計表示**し、件数の多い順に共通種別へワンクリックで昇格できる（`GET /api/admin/excluded-items/user-suggestions`・`suggested_type_id` 付き）。あわせて**サーバ登録対象外（`server_excluded_items`）**もこの画面で管理する
...
│   └── /admin/excluded-items # 表示種別・対象外の管理（種別割当・サーバ登録対象外・admin限定）
```
...
- name→種別の**ユーザー割当**。共通割当（`excluded_items`）が同名を持つ場合は共通が優先され、スナップショットでは共通に存在する名前を除いて返す（重複排除）。
...
- フロントは端末保存時に「対象外種別の割当」操作のたび `POST /api/excluded-items/report` で名前を fire-and-forget 送信する（失敗は致命でない）。送るのは名前のみ・匿名のため、ユーザー向けの追加注記は設けない
...
- 管理画面「表示種別・対象外の管理」の共通割当リストは、**種別ごとのタブ（「すべて」＋各種別。各タブに件数表示）で表示を切り替えられる**（フロント側の絞り込みのみ。`exclusion_type_id` が null の行は既定種別タブに含める）。全選択・一括削除は表示中タブ＋検索の絞り込み結果に対して働く
...
| `src/utils/inventory.test.ts` | アイテムボックスの行の実効種別判定 `effectiveTypeId`（取引可能＞共通割当＞ユーザー割当＞未設定・null は既定種別へ解決・名前正規化） |
