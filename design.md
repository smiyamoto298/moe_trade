...
- **表示**: 設定済みの場合、共通リンク部品 `OfficialDbLink`（`📖 公式DB ↗`）を**出品一覧（`ListingsPage` のアイテム名欄）・買取一覧（`BuyRequestCard`）・各詳細（`ItemInfoCard` 経由で出品詳細／買取詳細／アイテム詳細）**に表示し、クリックで**新しいウィンドウ**に開く。リンクは `<a>` の入れ子（Link カード内）を避けるため `button` + `window.open(..., '_blank', 'noopener,noreferrer')` で実装する。
