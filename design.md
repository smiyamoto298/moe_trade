...
- **表示種別（ジャンル）**: 旧「除外リスト」を廃止し、各アイテムを**種別（ジャンル）で分類して表示を切り替える**方式に変更（取り込みでは弾かず全件保持）。行の**実効種別**は `effectiveTypeId`（`frontend/src/utils/inventory.ts`）で次の優先順位で決まる: ①登録アイテムに紐づく（`itemId!=null`）→ **取引可能**（派生種別。ユーザーは変更不可）、②**ユーザーの種別割当**（`user_excluded_items` の name→`exclusion_type_id` / ローカル保存時はクライアント。`null` は既定種別「その他」）、③**共通の種別割当**（管理者・`excluded_items` の name→`exclusion_type_id`）、④どれも無し → **未登録**（内部値 `unset`）。**ユーザー割当は共通割当より優先**するため、ユーザーは自分のアイテムボックス上で共通種別を**自分用に上書き変更できる**（DB保存時もスナップショットでユーザー割当を共通と重複排除せず返す）。
  ...
  - **共通種別への昇格（共通種別設定）**: ユーザーの種別割当は管理画面で共通種別（`excluded_items`）へ昇格できる。昇格候補（`user-suggestions`）には**ユーザーが最も多く割り当てた種別**（`suggested_type_id`）を併せて返し、その種別でワンクリック昇格できる。**共通種別として登録された名前は、ユーザー個別の種別割当から削除される**（共通優先・重複排除。スナップショットでも共通に存在する名前はユーザー割当から除いて返す）。**端末（ローカル）保存ユーザーの割当は名前のみ匿名で `reported_excluded_names` に報告**し、昇格候補に DB保存分とマージして表示する（誰が分類したかは記録しない）。
...
- name→種別の**ユーザー割当**。ユーザー割当は共通割当（`excluded_items`）より**優先**される（ユーザーが共通種別を自分用に上書きできる）。そのため共通と同名でもスナップショットから除外せずそのまま返す。共通へ昇格した名前は昇格時に `user_excluded_items` から削除されるため通常は重複しない。
...
- `user-suggestions`（共通種別への昇格候補）に DB保存分とマージして `from_device=true` で表示する。各候補には**ユーザーが最も多く割り当てた種別** `suggested_type_id`（明示割当が無ければ null）を併せて返し、その種別でワンクリック昇格できる
...
- `GET    /api/admin/excluded-items/user-suggestions` — ユーザー割当を集計した共通種別への昇格候補（admin。`[{ name, user_count, from_device, suggested_type_id }]`・user_count 降順→name 昇順）。DB保存分は人数を `user_count` に集計、端末保存ユーザーの匿名報告分は `from_device=true` で合流（人数は持たないため `user_count=0` もあり得る）。`suggested_type_id` はユーザーが最も多く割り当てた種別（明示割当なしは null）。共通登録済み・却下済みの名前は除外
...
- `POST   /api/admin/excluded-items` — 追加（admin。`names[]` で改行区切り一括登録可・重複は無視。`exclusion_type_id` 任意＝省略時は既定種別「その他」）。**昇格した名前は `user_excluded_items` から削除**（重複排除）
