...
- **レシピ**: レシピ名（`recipe_name`）とバインダー（`recipe_binder`）を入力する。バインダーは付加効果の項目名と同じ仕組みで候補管理する（`binder_labels` テーブル・公開候補API・管理画面で編集／登録時に新規バインダー名を自動候補化）。

その他種別のアイテムは追加効果・付加効果・染色・ミスリル・必要スキル値・特殊条件を持たない。
出品一覧・管理画面では「その他」タブ（`item_type=other`）で表示が切り替わり、効果列が「情報」列（ペット名／バインダー／レシピ名）になる。買取一覧は種別タブを持たないためアイテム名で横断検索する。
