...
### その他
既存種別に当てはまらないアイテムの受け皿。最上位カテゴリ「その他」の子カテゴリ「未開封ペット」「レシピ」「ペット用アイテム」「アイテムセット」を種別として選択する。
...
出品一覧・管理画面では「その他」タブ（`item_type=other`）で表示が切り替わり、効果列が「情報」列（ペット名／レシピの各エントリ＝レシピ名・そのレシピ名の必要スキル値／ペット用アイテムの対象ペット・効果／アイテムセットのアイテムリスト）になる。アイテムセットのアイテムリストは**1つの枠内に1行1アイテムで改行表示**する（一覧の情報列 `OtherInfoCell`・アイテム詳細 `ItemInfoCard` とも同じ表示）。買取一覧は種別タブを持たないためアイテム名で横断検索する。
...
（テクニックの子: ノアピース・秘伝の書。その他の子: 未開封ペット・レシピ・ペット用アイテム・アイテムセット。装備セット・アセットは子を持たない特殊カテゴリ）
...
| set_items | JSON | その他「アイテムセット」: 構成アイテム名の文字列配列（自由入力・複数）。保存時に空エントリを除去し全て空なら NULL |
