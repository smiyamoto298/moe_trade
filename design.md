...
- **共通除外の種別（カテゴリ）と適用選択**: 共通除外は種別（`exclusion_types`・既定「その他」）で分類される。ユーザーは個別除外リストとは**別の「共通除外の設定」ボタン／モーダル**で適用範囲を選べる（**端末ローカル保存**・未設定時は admin が種別ごとに設定した**デフォルトON/OFF**（`default_enabled`）に従う。マイペ整理と一括出品で共通）。
  - **その他以外の種別は種別単位**でON/OFF（キー `moe_inventory_applied_exclusion_types`＝適用する種別ID。未設定時は `default_enabled=true` の種別を適用）
  - **デフォルトON/OFF**は admin が「除外アイテム管理」の種別管理で種別ごとに設定する（`exclusion_types.default_enabled`）。OFF にした種別は、未設定ユーザーには最初から適用されない（ユーザーは個別に上書き可）
...
- **マイページメニュー（ヘッダー）**: ログイン時、「マイページ」はドロップダウンで「取引リスト（`/mypage`）」「マイペ整理（`/mypage/items`）」へ遷移できる（管理メニューと同形式。モバイルはセクション展開）。**マイページバッジ**として未読チャット数を表示し、「最後の発言が相手」のチャットを未読とし、新規取引希望（メッセージ無しのチャット作成）も含む（バッジはドロップダウン見出しと「取引リスト」項目の両方に表示）
...
- 集計仕様: 出品・買取は対象期間（JST）に作成されたもの（取り下げ済み `cancelled` は除外）。
  ...
  同一アイテム・同一価格・同一取引方法は「×N」に集約（一括出品対策。即決と交渉可は別行）。取引件数は `trade_history` の有効分（`is_valid = true`）のみ
...
- **モーダルの描画位置（暗幕とヘッダー）**: 全画面オーバーレイ（`fixed inset-0` の暗幕）を持つモーダルは、ページ本文コンテナの `space-y-*`（隣接兄弟に `margin-top` を付与）の直下に置くと、`fixed` 要素が margin ぶん下方向にずれて上端（ヘッダーメニュー）が暗幕で覆われない不具合が出る。対策は2通り:
  ...
  - **ページ内に直書きしたオーバーレイ**は暗幕 div に `!mt-0` を付けて `space-y` のマージンを打ち消す（`OwnedItemsPage` / `admin/AdminItemsPage`）。`BulkListingPage` は本文ルートが `space-y` ではない（`mb-*` 間隔）ため対象外
...
### 操作案内ツアー（初回ガイド）
- 主要ページ（出品一覧 / 新規出品 / 一括出品 / 出品詳細 / マイページ / マイペ整理 / 新規登録）で、対象要素をスポットライトしながら順に説明する吹き出しツアーを表示する
...
├── /listings/bulk            # 一括出品（公式の所持アイテム一覧を貼り付けて登録。完了後は元居た画面へ戻る／戻り先が無ければ /mypage）
...
├── /mypage                   # マイページ
│   └── /mypage/items         # マイペ整理（アイテムボックス貼り付け・台帳管理・要ログイン）
...
| default_enabled | BOOLEAN | デフォルトON/OFF（既定 true）。まだ「共通除外の設定」をいじっていないユーザーにこの種別を既定で適用するか。admin が種別ごとに設定 |
| sort_order | INT | 表示順 |
| created_at / updated_at | TIMESTAMP | |
...
- `default_enabled` は admin が種別ごとに設定するデフォルトON/OFF。**ユーザーが「共通除外の設定」を未設定（端末ローカル未保存）の場合**、`default_enabled=true` の種別だけが既定で適用される（旧仕様の「全種別適用」は全種別 true 相当）。ユーザーが個別にON/OFFすればその選択が優先される。既定種別「その他」はアイテム単位で適用するため `default_enabled` の対象外
- ユーザーはマイペ整理で「適用する種別」を選べる（端末ローカル保存・未設定は `default_enabled=true` の種別を適用）。[フロント参照: `selectedCommonNames` / `getAppliedExclusionTypeIds`]
...
- 管理画面で候補を共通除外へ昇格するとき、**追加先の種別を選んでから昇格できる**（`POST /api/admin/excluded-items` に `exclusion_type_id` を渡す。未選択は既定種別「その他」）。個別の候補は行ごとの**「共通へ追加…」セレクトボックス**で種別を選ぶと、その種別で即座に共通化される。一括の「表示中をすべて追加」は上部の「一括追加先の種別」セレクトの種別で昇格する
- 管理画面「除外アイテム管理」の共通除外リストは、**種別ごとのタブ（「すべて」＋各種別。各タブに件数表示）で表示を切り替えられる**（フロント側の絞り込みのみ。`exclusion_type_id` が null の行は既定種別タブに含める）。全選択・一括削除は表示中タブ＋検索の絞り込み結果に対して働く
...
- `GET /api/mypage/item-counts` — 自分のアクティブ出品・買取の件数（item_id ごと。登録時の重複案内に使用）。併せて `listing_variants`（出品中の件数を `item_id:削れ:染色` 単位で集計）を返し、マイペ整理の「出品中」判定に使う
...
### 除外アイテム（貼り付け除外）
- （公開）`GET /api/excluded-items` — 共通の除外アイテムと種別。`{ types: [{ id, name, is_default, default_enabled, sort_order }], items: [{ name, type_id }] }`（`type_id` は null のとき既定種別 id に正規化）。クライアントは「適用する種別」（端末ローカル設定。未設定時は `default_enabled=true` の種別）で `items` を絞り込み、個別除外とマージする
