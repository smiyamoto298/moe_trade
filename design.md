...
出品一覧・管理画面では「その他」タブ（`item_type=other`）で表示が切り替わり、効果列が「情報」列（ペット名／レシピの各エントリ＝レシピ名・そのレシピ名の必要スキル値／ペット用アイテムの対象ペット・効果／アイテムセットのアイテムリスト）になる。買取一覧は種別タブを持たないためアイテム名で横断検索する。
