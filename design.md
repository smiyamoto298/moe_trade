...
- **締切日時の併記**: オークションは残り時間表示だけでなく**締切日時そのもの**を必ず確認できるようにする。出品一覧・買取一覧のカードとテーブル行では、残り期限ラベル（`remainingLabel`）の下に「M/D HH:mm締切」（`deadlineLabel`・閲覧者のローカルタイム）を併記する。詳細ページは従来どおり「締切: YYYY/M/D HH:mm:ss」のフル表示、マイ取引の出品中/買取中カードも同じ `deadlineLabel` 形式で表示する
