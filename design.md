...
### user_daily_accesses（日ごとのユニークアクセスユーザー記録）
| カラム | 型 | 説明 |
|---|---|---|
| id | BIGINT PK | |
...
※ `(user_id, date)` に UNIQUE、`date` に index。timestamps なし。`RecordDailyAccess` ミドルウェア（api グループ）が認証済みリクエストごとに `insertOrIgnore` で記録し、利用状況解析の「アクセス」系列（日次ユニークアクセスユーザー数）の集計元になる。
