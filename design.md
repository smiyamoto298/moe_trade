...
- **表示種別（ジャンル）**: 旧「除外リスト」を廃止し、各アイテムを**種別（ジャンル）で分類して表示を切り替える**方式に変更（取り込みでは弾かず全件保持）。行の**実効種別**は `effectiveTypeId`（`frontend/src/utils/inventory.ts`）で次の優先順位で決まる: ①登録アイテムに紐づく（`itemId!=null`）→ **取引可能**（派生種別。ユーザーは変更不可）、②**ユーザーの種別割当**（`user_excluded_items` の name→`exclusion_type_id` / ローカル保存時はクライアント。`null` は既定種別「その他」）、③**共通の種別割当**（管理者・`excluded_items` の name→`exclusion_type_id`）、④どれも無し → **未登録**（内部値 `unset`）。**ユーザー割当は共通割当より優先**するため、ユーザーは自分のアイテムボックス上で共通種別を**自分用に上書き変更できる**（DB保存時もスナップショットでユーザー割当をそのまま返す）。ただし**ユーザー割当が共通と同じ種別になった場合は冗長なので取り除く**（共通に従う＝該当設定は削除。保存時・スナップショット取得時の両方で除外。種別が異なる「上書き」は保持）。フロントは割当時に共通と同じ種別を選んだら個別設定を作らず、読込時にも既存の冗長な個別設定を取り除く（端末保存分のクリーンアップ）。
  ...
  - **一覧の種別列・種別変更ボタン**: 一覧に種別列を設け、行の実効種別をバッジ表示する。**取引可能（登録済み）以外の行は種別バッジがクリック可能**で、押すと種別選択ダイアログを開いて種別を変更できる（未設定の行は「未設定」、共通割当の行は種別名＋「共通」マーク、ユーザー割当の行は種別名＋✎ を表示）。**取引可能の行は固定**（変更不可）。**管理者は新規種別を登録可**＝`createType`、それ以外は既存種別から選択。割当はアイテム名単位で同名の全行に効き、ダイアログから変更・解除できる。共通割当の行も**ユーザーが自分用に上書き**でき（ユーザー割当が共通より優先）、解除すると共通割当に戻る。
...
| `src/utils/inventory.test.ts` | アイテムボックスの行の実効種別判定 `effectiveTypeId`（取引可能＞ユーザー割当＞共通割当＞未設定・null は既定種別へ解決・名前正規化） |
