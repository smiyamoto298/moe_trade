...
- **サイドバナー**: 本文（max-w-7xl）の左右余白に `public/img/side_banner/` のバナーを縦中央固定表示し、公式サイトへリンク。左=`moe_h_01.gif`（幅120px・画面幅1590px以上）、右=`moe_h_02.gif`（幅160px・1670px以上）。幅が足りない間は狭い左バナーのみ→両方の順に段階表示
...
- 管理画面5ページ（`/admin/*`）は `App.tsx` で `React.lazy` によるルート単位の遅延読み込み。
