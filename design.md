...
- アイテム一覧・検索・確認済みフラグ管理（**「装備品」「テクニック」「アセット」「その他」タブで切り替え**。各タブに未確認件数バッジ）
...
**「新規出品のアイテム」「新規買取」「取引件数」** をまとめたツイート文面を自動生成する。
...
- **サイトマップ**: `GET /sitemap.xml`（`SitemapController`・Laravel `routes/web.php`）が動的生成。
  一覧ページ（`/listings` `/skills` `/assets` `/buy-requests`）＋ **`verified_status=verified` の全アイテムの恒久ページ `/items/{id}`**
...
- **ページごとの `<title>` / meta description / canonical / JSON-LD / noindex**: `frontend/src/hooks/usePageMeta.ts`。
  GooglebotはJSレンダリング後のこれらをインデックスに使うため、SPAのままクライアント側で設定する。
...
### 出品一覧のタブとルーティング
- `/listings`（装備品）・`/skills`（テクニック）・`/assets`（アセット）・`/others`（その他）は同一の `ListingsPage` コンポーネントを `mode` プロップ（`'equipment' | 'skill' | 'asset' | 'other'`）で切り替える
- ルートごとに React の `key`（`"equipment"` / `"skill"` / `"asset"` / `"other"`）を付与し、タブ切り替え時に確実に再マウントさせる（検索パラメータやフィルター状態が古いまま残らないようにするため）
...
- **種別切替タブには各種別の出品件数バッジを表示**する。件数は `GET /api/listings/counts`（認証不要）が `{ equipment, technique, asset, other }` で返す。集計条件は一覧と同じ公開条件（`status` が `active`、`include_completed=1` で `completed` も含む／凍結ユーザーの出品は除外）で、`include_completed` 以外の絞り込みは反映しない（タブごとの総件数）。フロントは「取引完了を含める」トグルに追従して取り直す
...
├── /items/:id                # アイテム恒久ページ（相場・出品/買取への導線・SEOの正規ランディング先）
...
- 管理者が「共通にしない」と却下した名前。`user-suggestions`（共通除外への昇格候補）から除外し、何人が個別除外していても候補に再表示しない。個別除外（`user_excluded_items`）自体には影響しない
...
- `GET  /api/items/:id` — 詳細
- `POST /api/items` — 登録（全ログインユーザー。unverified で作成。`skill_requirements`、アセット項目 `placement` / `asset_width` / `asset_height` / `storage_count` / `special_function`、その他項目 `pet_name` / `recipe_name` / `recipe_binder` 対応。新規バインダー名は `binder_labels` へ自動追加）
...
- `GET  /api/listings/counts` — 種別タブ用の出品件数（認証不要）。`{ equipment, technique, asset, other }` を返す。一覧と同じ公開条件で集計し、`include_completed=1` で `completed` も含む。`include_completed` 以外の絞り込みは反映しない
...
### 除外アイテム（貼り付け除外）
- （公開）`GET /api/excluded-items` — 共通の除外アイテム名の配列（ローカル保存ユーザーも取得して個別除外とマージ）
- `GET    /api/admin/excluded-items` — 全件（id付き・admin）
- `GET    /api/admin/excluded-items/user-suggestions` — ユーザー個別除外（DB保存分）を名前で集計した共通除外への昇格候補（admin。`[{ name, user_count }]`・user_count 降順。共通除外済み・却下済みの名前は除外）
...
- `POST   /api/admin/excluded-items` — 追加（admin。`names[]` で改行区切り一括登録可・重複は無視）
- `PUT    /api/admin/excluded-items/:id` — 編集（admin）
