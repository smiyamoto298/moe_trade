...
- **投稿はX APIを使わない**（無料枠が2026年2月に廃止されたため）。各文面の「Xでポスト」ボタンが
  ...
  - **スマホ（iOS / Android）ではX公式アプリで開く**: UA判定でモバイルの場合、ディープリンク `twitter://post?message=...` でアプリの投稿画面を開く。アプリ未インストール等で開けない場合は1.5秒後にWeb Intentへフォールバックする（アプリが開いてページが非表示になればフォールバックは中止）。デスクトップは従来どおりWeb Intentを新規タブで開く
- **文字数制限と自動分割**: 未認証アカウントの上限（重み280＝全角140字相当。CJK=2・半角英数=1・URL=23換算）に収まるよう、
  アイテムが多い場合は複数ツイートへ自動分割し**全アイテムを漏れなく掲載**する
  - 2通目以降は**1通目への返信として投稿する前提**（スレッド化）。サイトURLとゲーム名タグ `#MasterofEpic` は1通目にのみ付け、共通タグ `#MoETrade` は全ツイートに付ける
