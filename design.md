...
- **宣伝ポストの対象**: オークションは**出品時**（`created_at`）と**入札で現在価格が更新されたとき**（`bumped_at` を `now()` に更新）に宣伝ポストの対象に含める（宣伝ポストは `COALESCE(bumped_at, created_at)` で対象期間を判定。`App\Support\Auction::refreshOutbid` が入札時に `bumped_at` を更新）
