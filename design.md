...
- **ヘッダーのナビ構成**: トップ階層に `出品一覧` / `買取一覧` / `アイテム一覧`（`/items`・全ユーザー。`unverifiedItemCount` のバッジ付き）を並べ、ログイン時はその後ろに `マイ取引`（`/mypage`）/ `アイテムボックス`（`/mypage/items`）/ `お問い合わせ`（管理メニューの直前）を表示する。マイページ階層・アイテム一覧は以前ドロップダウン配下だったがトップ階層へ移動済み
