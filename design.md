...
- **ローカル開発環境**: Docker Compose（nginx / php-fpm / **scheduler** / React dev / MySQL / Mailpit / phpMyAdmin）。`scheduler` サービスは `php artisan schedule:work` を常駐させ、定期バッチ（`auctions:resolve`=15分ごと / `listings:expire`=毎時 / `announcements:purge-expired`=毎日6:00）をローカルでも自動実行する
- **ホスティング**: さくらのレンタルサーバー（共用・スタンダード）。本番は **Apache + .htaccess** 構成、公開URL `https://moe-trade.sakuraweb.com`
- **デプロイ**: 手順書・自動化スクリプト一式は本番サーバーの実情報を含むため**リポジトリ外の非公開運用ドキュメント（`OPERATIONS.md`）で管理**（Git 追跡しない。「デプロイ・運用（非公開管理）」章参照）
...
- **WarAge無効**（`item_bonus_effects.no_warage_effect`）: 付加効果ごとのフラグ。各付加効果に「WarAge無効」チェックを設け、チェック時は付加効果の説明の末尾に「※WarAgeでは効果がない」を付けて表示する（文言は説明本文には保存しない）。装備品・装備セット部位とも共通
...
- 取引方法: 即決 / 交渉可 / **オークション**（後述「3-C. オークション」）
- 通貨: AC（固定）
- **削れあり**: 耐久度に削れがある中古品かどうかを出品ごとに設定（`listings.is_worn`）。出品フォームのチェックボックスで指定
- **染色済み**: 染色液で色を変更済みかどうかを出品ごとに設定（`listings.is_dyed`）。削れと同様、出品フォーム（出品・編集）のチェックボックスで指定し、出品一覧にも「🎨 染色済み」バッジを表示
- 出品期限は7日間。期限切れは自動取り下げ、マイページから再出品可能
- 出品フォームに期限の注意書きを表示（「7日間で期限切れ／マイページから延長可能」）

> **一括出品は廃止**（旧 `/listings/bulk`）。所持アイテムの貼り付け・管理はアイテムボックス（後述）に集約し、出品は単品出品（`/listings/new`）で行う。
...
- **一覧は出品一覧と同様のテーブルレイアウト**（アイテム／価格／取引／操作列。`listing-table-container` のコンテナクエリで狭い幅では取引列を畳む）。買取一覧では装備性能・確認ステータスは表示せず、アイテム名（種別・公式DBリンク）・取引条件・価格のみを表示する
  - 各行の操作列に**「取引」ボタン**（押すと行下に取引希望パネル `TradeRequestPanel`（`kind=buy_request`）を展開して売却を申し出る。未ログイン・メール未認証・自分の買取・申し出済み（`GET /api/mypage/selling-offers` で判定）の行には表示しない。申し出済みは「✓ 申出済み」を表示）と、
    **「相場情報」ボタン**（PC・広い幅のみ。`PriceAnalyticsModal` をポップアップ表示）／**「詳細 →」リンク**（狭い幅のみ。買取詳細へ）を用意する（出品一覧と同じ構成）
  - 出品一覧と同じ**表示モードトグル（詳細 / シンプル）**を持つ（「共通UX仕様」の該当項目を参照。保存キーも共通）
  - コメントがある買取はアイテム行の直下にコメント行を表示する
  - 申し出対象が取り下げ／成立済みだった場合は一覧上部にエラーバナーを表示して一覧を再取得する（買取詳細からのリダイレクト `state.tradeError` も同バナーで表示）
- 一覧のソート：新着順 / あいうえお順（アイテム名昇順 `name_asc`）/ 買取価格が高い順 / 買取価格が安い順（新着順は `COALESCE(bumped_at, created_at)` 降順。値下げ／即決→交渉可で再登録した買取が新規同様に先頭へ来る）
...
- 取引方法に**オークション**を選べる（出品と共通仕様。後述「3-C. オークション」）

### 3-C. オークション（取引方法 `auction`）
出品・買取に共通の取引方法。価格競争で取引相手を決める。出品=高い入札ほど有利／買取=安い入札ほど有利。

- **設定項目**: 最低取引価格（出品）／最高取引価格（買取）＝`price`（開始価格 兼 下限/上限）、**即決価格**＝`buyout_price`（任意。出品は `price` より高く／買取は低く）、**期限日**＝`expires_at`（必須・未来日時・最長30日）。期限日は解決バッチ（15分ごと）に合わせ**15分単位**にする。フォームは1分単位の自由入力を避けるため**日付ピッカー＋15分刻みの時刻プルダウン**（`DeadlineInput` コンポーネント。00:00〜23:45 の96択）で入力し、サーバー側でも `App\Support\Auction::roundDeadline` で15分単位へ丸めて保存する（最終保証）
- **入札**（取引希望）: 取引希望時に入札額（`bid_price`）を設定する。出品は `price` 以上かつ現在の最高入札より高いこと、買取は `price` 以下かつ現在の最安入札より安いことが条件。**条件を満たさない入札は拒否**（`400`）。**入力中に他ユーザーがより有利な額で入札して抜かれた場合**も同じ `400` で拒否し、レスポンスに最新の `current_price`／`best_bid` を含める。取引希望パネル（`TradeRequestPanel`）はこれを受けて**現在価格を最新に更新し、その額を添えたエラー**（「他のユーザーがより高い／安い額で入札しました。現在の入札額は XX AC です。」）を表示して、一覧へ飛ばさず**その場で再入札**できるようにする
- **入札の更新**: 入札側はマイ取引から**より有利な額のみ**更新できる（`POST /api/chats/:id/bid`）。**取り下げ（撤回）はできない**
- **即決**: 入札が即決価格に達した時点で（期限を待たず）**即時成立**する
- **期限日の自動解決**（日次バッチ `listings:expire`）: 通常の期限切れ自動取り下げは適用しない。期限日時点で
  - 入札あり → **最良入札（出品=最高／買取=最安）が自動的に取引成立**（同額は先着優先）
  - 入札なし → **自動取り下げ（`expired`）**。マイ取引から取り下げ／再出品（最低取引価格を下げる）できる
  - 解決バッチは**専用コマンド `auctions:resolve` を15分ごと**に実行する（通常の即決/交渉可の期限切れ `listings:expire`（毎時）とは分離）。期限日を15分単位に丸めているため、締切到来後ほぼ遅延なく成立/取り下げになる
- **取り下げ・編集の禁止**: 入札が1件でもあると、出品/買取の取り下げ・編集はできない（自動成立するため）
- **入札なしで終了した場合**: 入札ゼロで `expired` になったオークションは、マイ取引から**取り下げ**または**再出品**できる。再出品では**最低取引価格を下げる**（買取は最高取引価格を上げる）ことが必須（不人気だったため有利な条件にする）。即決価格・期限日も再設定し、`bumped_at` を更新して新着・宣伝対象にする（`renew` がオークション時は専用処理）
- **期限切れ扱いにしない**: オークションは期限到来後にバッチで自動成立/取り下げされるため、通常の「期限切れ（再出品促し）」バケットには含めない（`Listing::scopeExpired`・通知の `expired_count`・マイ取引の `isExpired` から除外）。期限到来〜バッチ確定までの間、出品者のマイ取引には「⏳ オークション締切・集計中」と現在価格を表示し、確定後に「🎉 XX ACで落札されました」または「入札が無いまま終了しました」に切り替わる
- **owner の手動操作禁止**: オークションは自動成立のため、owner からの手動成立（deal）・見送り（decline）はできない
- **owner 側の表示**: 出品者/買取登録者は成立まで各入札チャットを見られず、マイ取引には**現在の入札価格（現在価格・入札件数）のみ**を表示する。落札確定後（自動成立）に限り、**「🎉 XX ACで落札されました」**の成立メッセージと落札者とのチャットを表示し、受け渡し（TELL・完了確認）を進める。成立した落札チャットは通常どおり**未読通知の対象**になり、出品中／買取中タブのバッジと該当記録のハイライト（通知アイコン）で出品者に知らせる（open の入札は通知に出ない）
- **価格更新通知**: より有利な入札が入って順位が下がった**他の入札者**へ通知する（`outbid_at` を記録し、通知サマリーの `outbid_chats` で配信）
- **宣伝ポストの対象**: オークションは**出品時**（`created_at`）と**入札で現在価格が更新されたとき**（`bumped_at` を `now()` に更新）に宣伝ポストの対象に含める（宣伝ポストは `COALESCE(bumped_at, created_at)` で対象期間を判定。`App\Support\Auction::refreshOutbid` が入札時に `bumped_at` を更新）。文面では【新規の取引】とは別の**【オークション現在価格】**セクションに現在価格で載る（→ 10-B）
- **落札落選の通知**: 他のユーザーが落札して自分の入札が不成立（`declined`）になった入札者には、マイ取引上部に「オークション『〇〇』は他のユーザーが落札しました（あなたの入札: XX AC）」の通知を表示。**「確認済みにする」**で個別に非表示にできる（既読は localStorage `mypage_auction_lost_seen` で管理）
- **締切カウントダウン**: マイ取引の「取引希望」「販売希望」タブの各入札チャットに、締切（`expires_at`）までの残り時間を毎秒更新で表示する（1日以上は「残り N日H時間M分」、未満は「残り HH:MM:SS」）。残り時間だけでは締切がいつか分からないため、締切前は「・M/D HH:mm締切」を併記する
- **締切日時の確認手段**: オークションは残り時間表示だけでなく**締切日時そのもの**を確認できるようにする。出品一覧・買取一覧（カード `ListingCard` とテーブル行 `ListingsPage` / `BuyRequestsPage`）では、残り期限ラベル（`remainingLabel`）に**カーソルを当てると**「締切: YYYY/M/D HH:mm」（`deadlineTooltip`・閲覧者のローカルタイム）をツールチップ（`title` 属性）で表示する。一覧の情報密度を上げないための hover 表示なので、非オークションには付けない（`deadlineTooltip` が `undefined` を返す）。ホバーできることを示すため、オークション行のみカーソルを `cursor-help` にする。詳細ページは従来どおり「締切: YYYY/M/D HH:mm:ss」のフル表示、マイ取引の出品中/買取中カードは「M/D HH:mm締切」を常時表示する
- **出品時の注意喚起**: フォームで「条件（即決価格到達・期限日）を満たすと自動的に取引成立すること」「入札後は途中で取り下げできないこと」を明示する
...
- **その他タブ**: 未開封ペット・レシピを表示。レシピの**必要スキル値での絞り込み**に対応（テクニックと同じ `skill_keys` / `skill_ranges`。バックエンドの必要スキル値フィルターは種別非依存のため `item_type=other` でもそのまま機能する）。レシピはエントリ（レシピ名）ごとに必要スキル値を持つが、検索は集約値 `items.skill_requirements`（各スキル最大値）に対して行うため「**いずれかのエントリが該当すればヒット**」となる（※数値上限フィルタ `skill ≤ X` は最大値集約のため稀に取りこぼす近似）。マスタリ概念のあるテクニック専用の検索モード切替（通常/構成）・マスタリ込みオプションは表示しない
...
- ソート：新着順 / あいうえお順（アイテム名昇順 `name_asc`）/ 価格昇順 / 価格降順（新着順は `COALESCE(bumped_at, created_at)` 降順。値下げ／即決→交渉可で再出品した取引が新規同様に先頭へ来る）
...
- **メッセージ履歴の日付セパレーター**: 各メッセージには時刻（HH:mm）のみ表示するため、履歴の先頭と日付が変わる位置に日付セパレーター（例: `2026年6月12日(金)`。年月日＋曜日、ローカルタイムゾーンの暦日で判定）を表示する。同日内のメッセージ間には表示しない
- **メッセージの編集・削除**（マイページの取引チャット）
  - **編集**: 自分のメッセージのうち**チャット内で最新の1件のみ**（`PATCH /chats/:id/messages/:messageId`）。相手が返信した後に文脈を書き換えられないようにするため
  - **削除**: 自分のメッセージのみ（`DELETE /chats/:id/messages/:messageId`）。ただし**最初の取引希望メッセージ（チャット先頭の1件）は取引の起点となる記録のため削除不可**
  - いずれもメッセージ送信と同じ状態制約（open / deal のみ。クローズ済み・他ユーザーの取引成立後は不可）。他人のメッセージは 403、チャット外のメッセージIDは 404
  - フロントは吹き出し下（時刻の横）に「編集」「削除」リンクを表示。編集はインライン入力（Enter=保存 / Escape=キャンセル）、削除は確認ダイアログ後に実行。編集は最新メッセージのみ可能なため、5秒ポーリングの変化検知は末尾メッセージの本文比較で相手側の編集を反映する
...
- **「Xで検索」ボタン**：右上に表示し、クリックでアイテム名を X（旧Twitter）で検索する新規ウィンドウ（`https://x.com/search?q=<検索語>&src=typed_query`・`target="_blank"` / `rel="noopener noreferrer"`）を開く。アイテム名はモーダルの `itemName` プロップ／出品・買取・アイテム詳細ページの `item.name` から `PriceAnalytics`（経由は `PriceAnalyticsAsync`）に渡す（未指定時は非表示）。検索語はノイズ低減のため `PriceAnalytics` の `toXSearchQuery` で整形する：`秘伝の書 [ ○○ ]`／`ノアピース [ ○○ ]` は括弧内（○○）だけを取り出し、空白（全角・半角）はすべて除去する
- 表示は共通モーダル `frontend/src/components/PriceAnalyticsModal.tsx`（出品登録・アイテムボックスの「相場情報」ボタンから利用）と出品詳細ページで共有
...
### 7. マイページ
- 出品中タブ：出品管理（期限更新・再出品・取り下げ）＋各出品のチャット一覧。期限切れの再出品・再登録はモーダルで価格（現在価格を初期値）と取引方法（即決／交渉可）を設定し直してから実行する。期限切れ一覧の各カードには再出品/再登録に加えて取り下げボタンを置く
- 期限切れの出品・買取がある場合、ページ先頭に警告バナーを表示し、「期限切れをすべて取下げ」ボタンで対象一覧を確認ダイアログに出して一括取り下げできる。ヘッダー「マイ取引」にも期限切れ件数の通知バッジ（琥珀色。未読チャットの赤バッジとは別）を表示する
...
- キャラクター管理（追加・変更・削除）
- **ヘッダー行は見出しのみでボタンを置かない**。「⚙️ 通知設定」（`/mypage/notifications` へ遷移）はサイトヘッダー右側へ移動した（後述「レイアウト・ブランド表示 > ヘッダー右側の操作」）。**ブラウザ通知の許可・ON/OFF・ブロック解除案内はすべて通知設定画面に集約**し、マイページ側には状態表示も操作も置かない（旧「🔔 ブラウザ通知を有効にする」「🔔 通知ON／OFFにする」「🔕 通知OFF — ONに戻す」「🔕 通知がブロックされています（解除方法）」は §8 の通知設定画面へ移設）
...
#### アイテムボックス（`/mypage/items`・要ログイン）
公式サイトのアイテムボックスを貼り付けて「自分が所持しているアイテム」を記録・管理する台帳。出品とは独立。
貼り付け解析は共通トークナイザ `frontend/src/utils/itemBoxPaste.ts` の `parseItemBox` を使う。
- **MoE アカウント**: 公式のアイテムボックスはアカウント単位のため、所持品整理用のアカウント名を複数登録できる（取引用キャラクター `user_characters` とは別概念）。アカウントごとに貼り付け、表示は「すべて／アカウント別」を**タブ（横並びのボタン）で切替**（各タブに件数を併記。旧データの未割り当てが残る場合のみ「未割り当て」タブも表示）。**タブ数（「すべて」「未割り当て」を含む）が 8 を超えたら、`lg` 以上でもドロップダウン（`<select aria-label="表示アカウント">`）に切り替える**（フィルタ行は画面上部に sticky 固定するため、アカウントが増えてピル型タブが何段にも折り返すとフィルタ行が肥大化し、一覧の表示領域を覆い隠してしまう。`lg` 未満は件数に関わらず従来どおりドロップダウン）。**所持アイテムは必ずいずれかのアカウントに属する（未割り当ては許容しない）**。アカウント削除時は紐づくアイテムも削除する（旧データに残る未割り当ては表示・閲覧のみ可）。**アカウントの追加・改名はブラウザ標準の `window.prompt` ではなく、ダイアログ内のテキストボックスで名前を入力させる**（共通ダイアログ `DialogContext` の `prompt`。タブ非アクティブ時に標準プロンプトが抑制される問題を回避し、`confirm`／`alert` と UI を統一）。**アカウントの管理（追加・改名・削除）はページヘッダー（保存先トグルと同じ行）の「アカウント管理」ボタンから開くモーダルで行う**（アカウント一覧の専用カードは廃止。アカウント別の件数は表示切替タブ・取り込み先ピルで確認できる）。**アカウント名は所持品を分けるための管理用ラベルにすぎず、実際のアカウント名と一致していなくてよい旨を注記する**（名前入力ダイアログ・アカウント管理モーダル・操作案内ツアーの3か所。公式サイト側と照合するわけではないため。DB保存時にゲームIDではなくニックネームを推奨する既存の注意書きと併せて表示する）
- **貼り付け取り込み**: 取り込み先アカウントを選んで貼り付け→`parseItemBox`→`POST /api/items/match` で登録アイテムと照合（**未紐づけでも保存可能**）。**一覧を表示するタイミング（ページ表示時）に、各行の名称を `POST /api/items/match` で登録アイテムへ再照合する**。未紐づけ（`itemId=null`）の行は一致したものを**自動でリンク**（取り込み時点では未登録だったアイテムが後から新規登録された場合に、ページを開き直すだけで紐づく。末尾「...」の省略名は誤紐づけ防止のため対象外）。**紐づけ済みの行は登録アイテム名で照合し、登録アイテムの最新情報でスナップショットを更新する**（ローカル保存では `item` がリンク時点の凍結スナップショットのため、`official_url`（公式DBリンク）など後付けの項目が反映されない問題への対処。照合は保存名ではなく登録アイテム名で行う）。**リンク／更新で内容が変わった行があるときだけ保存する**（無変更なら保存しない）。**読込時にアカウント未選択ならその場で作成を促す（未割り当てでは取り込まない）**。転送×（トレード不可）は取り込まない。**それ以外は全件取り込む**（除外はせず、種別で表示を切り替える方式）。取り込みは**全て置き換え**に統一（読込のたびに対象アカウントの行を貼り付け内容で置換）。マーク／削れ／染色は同名行から引き継ぐが、**同名アイテムが複数ある場合は出現順（n番目）で個別に対応づけ**、削れ・染色を行ごとに別々に管理できる。**同名アイテムの数が減り、かつ既存行のステータス（削れ・染色・マーク）が複数種類あるときは、どのアイテム（ステータス）を残すかを確認モーダルで選ばせてから取り込む**（どれを捨てるか曖昧なケースのみ。状態が同一なら確認しない）。**貼り付け領域はアコーディオンで折りたためる**（見出し行のクリックで開閉。開閉状態は端末ローカル保存・キー `moe_inventory_paste_open`・**既定は閉じる**（開いた選択のみ保存値 `'1'` で残す））。見出し「アイテムボックスを貼り付け」の右側に「※再取り込みすると、貼り付けた一覧にないアイテムは自動で削除されます。」の説明書きを常時表示する（全て置き換え取り込みであることの注意喚起）
- **表示種別（ジャンル）**: 旧「除外リスト」を廃止し、各アイテムを**種別（ジャンル）で分類して表示を切り替える**方式に変更（取り込みでは弾かず全件保持）。行の**実効種別**は `effectiveTypeId`（`frontend/src/utils/inventory.ts`）で次の優先順位で決まる: ①**ユーザーの種別割当**（`user_excluded_items` の name→`exclusion_type_id` / ローカル保存時はクライアント。`null` は既定種別「その他」）、②**共通の種別割当**（管理者・`excluded_items` の name→`exclusion_type_id`）、③登録アイテムに紐づく（`itemId!=null`）→ **取引可能**（派生種別）、④どれも無し → **未登録**（内部値 `unset`）。**登録アイテムに紐づく行でも、種別の割当（ユーザー／共通）があればそちらを優先**する（アイテム情報の登録による「取引可能」は割当が無いときの既定。割当を解除すると取引可能に戻る）。**ユーザー割当は共通割当より優先**するため、ユーザーは自分のアイテムボックス上で共通種別を**自分用に上書き変更できる**（DB保存時もスナップショットでユーザー割当をそのまま返す）。ただし**ユーザー割当が共通と同じ種別になった場合は冗長なので取り除く**（共通に従う＝該当設定は削除。保存時・スナップショット取得時の両方で除外。種別が異なる「上書き」は保持）。フロントは割当時に共通と同じ種別を選んだら個別設定を作らず、読込時にも既存の冗長な個別設定を取り除く（端末保存分のクリーンアップ）。
  - **種別タブ行**: アカウント切替の下に、アカウントと同じ**単一選択タブ**で表示種別を切り替える行を追加する（`すべて / 取引可能 / 各共通種別… / 各カスタム種別… / 未登録`、各タブに件数併記）。タブ行末尾に「⚙ カスタム種別」ボタンを置き、カスタム種別の管理モーダルを開ける。選択は端末ローカル保存（キー `moe_inventory_display_type`。カスタム種別は `ct_` 付き id 文字列）。**既定は「取引可能」のみ**表示（他の種別は最初は非表示）。読込時に選択タブが既に存在しないカスタム種別を指していたら「取引可能」へ戻す。
  - **カスタム種別（ユーザーごと・自分専用）**: 共通種別（admin管理・全ユーザー共有）とは別に、**全ユーザーが自分専用の表示種別を追加できる**。クライアント表現の id は `ct_` プレフィックス付き文字列（`CustomTypeId`。ローカル生成キー、または DB保存時のサーバー id を `ct_${id}` にしたもの）で、共通種別の number 型 id と型レベルで区別する（`InventoryData.customTypes`）。割当は `UserTypeAssignment.custom_type_id`（設定時は `exclusion_type_id` より優先。カスタム割当は共通との冗長判定・共通化候補の匿名報告の対象外）。**保存先が「サーバー（DB）」のときはカスタム種別もサーバー（`user_exclusion_types`）に保存**され、他端末でも同じ種別が使える（ローカル保存時は `localStorage` の台帳と一緒に保存）。管理は「⚙ カスタム種別」モーダル（追加・改名・削除。名前は共通種別・カスタム種別の両方と重複不可・100文字以内）。**削除すると、その種別を割り当てたアイテム名の割当も一緒に解除**される（実効種別は共通割当／取引可能／未登録へ戻る）。種別選択ダイアログにもカスタム種別のチップ（自分専用）と「新しいカスタム種別名＋追加して割当」入力を表示する。
  - **一覧の種別列・種別変更ボタン**: 一覧に種別列を設け、行の実効種別をバッジ表示する。**どの行も種別バッジがクリック可能**で、押すと種別選択ダイアログを開いて種別を変更できる（未設定の行は「未設定」、共通割当の行は種別名＋「共通」マーク、ユーザー割当の行は種別名＋✎、取引可能の行は「取引可能」＋✎ を表示）。**取引可能（登録済み）の行にも種別を割り当てられ、割り当てた種別が取引可能より優先**される（解除すると取引可能に戻る）。**管理者は全ユーザー共通の新規種別を登録可**＝`createType`。**全ユーザーは自分専用のカスタム種別を追加して割当できる**（「+ 追加して割当」。上記「カスタム種別」参照）。割当はアイテム名単位で同名の全行に効き、ダイアログから変更・解除できる。共通割当の行も**ユーザーが自分用に上書き**でき（ユーザー割当が共通より優先）、解除すると共通割当に戻る。**アイテム情報が未登録（`item=null`）の行のダイアログには、アイテム登録への導線を表示**する（完全な名前は「＋ アイテム情報を新規登録」ボタン→`NewItemForm`、末尾「...」の省略名は「候補から登録」ボタン→`CandidateSelectModal`。押すとダイアログを閉じて各モーダルを開く。種別割当済みで行内に登録ボタンが出ない行でも、ここから登録できる）。
  - **共通種別への昇格（共通化）**: ユーザーの種別割当は管理画面で共通種別（`excluded_items`）へ昇格（共通化）できる。昇格候補（`user-suggestions`）には**ユーザーが設定した種別の内訳**（`type_assignments`：種別ごとの人数・多い順）と**最も多く割り当てた種別**（`suggested_type_id`）を併せて返し、内訳を見たうえで種別を選んでワンクリック共通化できる。候補は2種類: **新規候補**（未登録の名前）と、**上書き候補**（既に共通登録済みだが現在の共通種別と異なる種別を設定したユーザーがいる名前。`current_type_id` に現在の共通種別を返す）。上書き候補を共通化すると共通種別を更新する（API に `update_existing=true`）。**共通化した名前は、ユーザー個別の種別割当から削除される**（共通へ集約・重複排除）。**端末（ローカル）保存ユーザーの割当は名前のみ匿名で `reported_excluded_names` に報告**し、昇格候補に DB保存分とマージして表示する（誰が分類したかは記録しない）。
- **サーバ登録対象外（分割保存）**: 運営に見られたくないアイテムは、保存先が「サーバー」でも**そのアイテムだけサーバーへ送らず端末（ローカル）に保存**する。対象名は**システム共通分**（`server_excluded_items`・管理者が管理・公開 `GET /api/server-excluded-items`）と**ユーザー指定分**（端末ローカルのみ・キー `moe_inventory_server_excluded`・サーバーには送らない）の和集合。DBモードでの保存時、対象名の行は PUT ペイロードから除外して `localStorage`（キー `moe_inventory:db-local:v1`・アカウント名も保持）へ保存し、読込時に DBスナップショットへマージして一覧では統合表示する（分割保存ロジックは `frontend/src/utils/inventoryStore.ts`）。一覧の行から「端末のみ」トグルで個別指定でき、設定モーダルでも追加・解除できる。
- **重複を確認**: フィルタ行に「重複を確認」ボタン（バッジに該当名称数）を置き、**異なる取り込み先（MoE アカウント）で同じ名称のアイテムを所持しているもの**をモーダルで一覧表示する。**取引可能（登録アイテム）に限らず、種別が割り当てられた行も対象**とし、**実効種別が「未登録」（`unset`）の行のみ対象外**。判定は**表示名（`displayName`）を正規化したキー単位**で、所持アカウントが2件以上にまたがる名称を抽出し、各取り込み先の所持個数（`count` の合計）を併記する。表示は名称のあいうえお順
- **買取ありフィルタ**: フィルタ行（「★ マークのみ」の隣）に「💰 買取あり (件数)」トグルを置き、**他ユーザーが買取募集中（active）のアイテム**（`POST /api/buy-requests/prices` の結果に item_id が含まれる紐づけ済み行）だけに絞り込める。件数バッジには一覧表示時点で買取募集があるアイテムの件数を常時表示し、絞り込みは種別タブの件数にも反映する
- **一覧は常にあいうえお順**（日本語ロケール）で表示。並びの基準は表示名（登録アイテム名があればそれ、無ければ貼り付け名）
- **行ごとの情報・操作**: 登録アイテムに紐づく行は**登録アイテムの情報のみ表示**（名前・カテゴリ・追加効果。貼り付けの生データ名は出さない）。**紐づけ済み行のアイテム名はクリック可能で、押すとアイテム詳細モーダル（`frontend/src/components/ItemDetailModal.tsx`）をポップアップ表示**する（`GET /api/items/:id` で取得し、アイテム恒久ページ `/items/:id` と同じ基本情報（`ItemInfoCard`）・確認中バッジ・ハッシュタグ（読み取り専用）・相場（`PriceAnalytics`）をページ遷移せずに確認できる。フッターの「詳細ページを開く」リンクは未保存編集を失わないよう**新しいタブ**で恒久ページを開く。未紐づけ行の名前はクリック不可のテキスト表示のまま）。未紐づけの行は貼り付け名＋ボタンを表示（**末尾「...」の省略名は「候補」ボタンのみ**、完全な名前は「新規登録」ボタンのみ。省略名の「候補」ダイアログ（`CandidateSelectModal`・前方一致検索）には、**前方一致候補の有無に関わらず「新規登録」ボタンを常時表示**し、押すと検索キーワードを `NewItemForm` の初期名に引き継いで新規登録へ進む）。マーク（★）/「★マークのみ表示」、**削れあり・染色済み**のチェック、**出品**ボタン（`/listings/new` に `presetItem`／`presetWorn`／`presetDyed` を渡して初期選択・削れ／染色も初期チェック。**アイテム・削れ・染色がすべて一致する自分の募集中出品**があればボタンの代わりに「出品中」表示。判定は `GET /api/mypage/item-counts` の `listing_variants`（`item_id:削れ:染色` 単位の件数）。一致する出品が複数あれば「出品中 N件」と件数表示）、**相場情報**ボタン（`PriceAnalyticsModal`）、**他ユーザーが買取中の価格**を表示し**クリックで売却（買取詳細 `/buy-requests/:id`）へ遷移**（`POST /api/buy-requests/prices` で item_id ごとの最高額 active 買取を取得。複数あるときは最高値を表示し「最高 / 全N件」を併記）、**メモ**（行ごとの自由記入欄。最大500文字。入力は自動保存（デバウンス）され再読み込み後も残る。再取り込み時はマーク／削れ／染色と同様に同名行の出現順で引き継ぐ。保存先は台帳と同じく `local`／`db` に従う）
...
- **レスポンシブ（一覧のテーブル⇔カード切替）**: 一覧は **lg（1024px）以上ではテーブル、lg 未満ではカード型リスト**で表示する（スマホ・タブレットで横スクロールせずに全項目を確認・操作できる）。切替は CSS の出し分けではなく `useMediaQuery('(min-width: 1024px)')`（`frontend/src/hooks/useMediaQuery.ts`・matchMedia 購読）で**片方だけ描画**する（hidden 併記だとメモ入力等のフォーム要素が二重になるため。matchMedia が使えない環境（テストの jsdom 等）ではテーブル表示にフォールバック）。カードは「①マーク★＋アイテム情報＋買取中価格 ②種別バッジ・アカウント・個数・削れ/染色 ③メモ＋操作ボタン（出品/出品中・相場・端末のみ・削除）」の3段構成で、行の表示パーツ（アイテム情報・種別バッジ・買取価格・操作ボタン群）はテーブル行と共用のレンダリング関数で描画し機能差を作らない。**アカウント切替（表示）・種別切替のタブは lg 未満ではピル型タブではなくドロップダウン（`select`・aria-label「表示アカウント」「表示種別」）で表示**し、選択肢に件数を併記する（タブが多いと画面を占有するため。「⚙ カスタム種別」ボタンは種別ドロップダウンの隣に残す）。フィルタ行の1段目（アカウント切替＋マーク/買取あり/サーバ登録対象外/重複ボタン）は sm 未満で縦積みにする
- **表示切替バー・テーブルヘッダーの固定**: 一覧をスクロールしても操作しやすいよう、**表示切替（タブ）＋フィルタの行**と**テーブルのヘッダー行**を画面（ビューポート）上部に sticky 固定する。グローバルヘッダー（お知らせバナー込み）と表示切替バーの高さを `ResizeObserver` で実測し、CSS 変数 `--thead-top` で各 sticky の `top` オフセットに反映する。テーブルのヘッダー固定は横スクロール用ラッパー（`overflow-x-auto`）がスクロールコンテナ化して sticky を無効化するのを避けるため、横スクロールが不要になる **lg 以上でのみ** `overflow:visible` に切り替えて有効化する（lg 未満はカード型リスト表示のためテーブルヘッダー自体が無く、表示切替バーのみ固定）
...
- **マイページメニュー（ヘッダー）**: ログイン時、「マイページ」はドロップダウンで「取引リスト（`/mypage`）」「アイテムボックス（`/mypage/items`）」へ遷移できる（管理メニューと同形式。モバイルはセクション展開）。**マイページバッジ**として未読チャット数を表示し、「最後の発言が相手」のチャットを未読とし、新規取引希望（メッセージ無しのチャット作成）も含む（バッジはドロップダウン見出しと「取引リスト」項目の両方に表示）
- **お問い合わせバッジ（ヘッダー）**: 新着投稿があると赤ドット表示。admin は他人の全投稿、一般ユーザーは自分のスレッドへの返信が対象
...
- **通知設定画面（`/mypage/notifications`・ログイン必須）**: サイトヘッダー右側（ログイン時のみ表示）の「⚙️ 通知設定」から遷移。種別 × チャネルのトグル（変更即保存・失敗時は元に戻す）と、メール通知の宛先設定をまとめて行う。チャネル自体が無効な状態（ブラウザ通知が未許可／ブロック／OFF、通知先メールが未設定／未確認）のときは「ONにしても届かない」旨と復旧動線を表示する。画面の並びは **①PWAインストール導線（該当端末のみ・下記）→ ②ブラウザ通知の受け取り（このブラウザ）→ ③通知の種類（種別×チャネル）→ ④メール通知の宛先**
  - **PWAインストール導線（画面先頭）**: インストール可能（`beforeinstallprompt` 保持中＝Android Chrome 等で未インストール）かつヘッダー直下のインストールバナーを**「あとで」で消した端末**（localStorage `pwa_install_dismissed`）にのみ、「アプリをインストール」ボタンを表示して `promptInstall()` を呼ぶ。バナー表示中の端末はヘッダー直下に同じ導線があるため出さない。standalone 起動（インストール済み）でも出さない
  - **「ブラウザ通知の受け取り（このブラウザ）」セクション**（種別トグルより先頭に独立セクションとして表示。ここがONでないと種別をONにしても届かないため）で、このブラウザへの配信可否を操作する。下の種別トグルが「何を受け取るか」の設定なのに対し、こちらは「このブラウザに配信するか」。**マイページから移設したもので、以下は従来と同じ挙動**
...
- **PWA起動時の通知ブロック警告**（`components/PushBlockedBanner.tsx`）: standalone 起動（PWA）かつ通知API対応かつ許可が denied のとき、ヘッダー直下に「🔕 通知がブロックされているため、プッシュ通知は届きません」の警告バナーを表示し、「解除方法」ボタンで上記の共通ダイアログ（`buildPushGuide`）を開く。standalone はアドレスバーが無くブラウザの鍵アイコンで許可状態を確認できないうえ、iOS は Safari の許可がアプリへ引き継がれずインストール直後に気づかないままブロック状態になりやすいための導線。サイト側の設定で自ら通知をOFFにしている人（`pushOptedOut`）には出さない。✕で閉じられる（その起動中のみ・永続化しない）。回帰防止テスト: `frontend/src/components/PushBlockedBanner.test.tsx`
...
- **表示対象**（`target_type`）: `all`=全員（デフォルト・未ログイン含む） / `staff`=管理・編集者のみ / `specific`=指定ユーザーのみ。`specific` のときは `target_user_ids`（ユーザーID配列）に含まれるユーザーにのみ表示する。公開API は認証不要だが、Bearer トークンがあれば `sanctum` ガードで現在ユーザーを解決し、`Announcement::isVisibleTo()` でサーバー側絞り込みする（対象外には本文を返さない）。`staff`/`specific` は未ログインには表示されない。`target_type` が `all`/`staff` のときは `target_user_ids` を `null` に正規化して保存する
- **既読化**（`specific` 限定）: 指定ユーザー向けお知らせには「既読にする」ボタンを表示し、対象ユーザー本人が `POST /api/announcements/:id/read` で既読にできる。既読にすると本人を `target_user_ids` から外し、残りが 0 人になった時点でお知らせレコード自体を削除する。`all`/`staff` 向けは既読の対象外（端末ごとの localStorage「表示しない」で対応）で、本エンドポイントは 403 を返す
...
- admin が `/admin/announcements` で作成・編集・削除・並び替え（`POST /admin/announcements/reorder`）。`specific` の対象ユーザーは管理画面で複数選択する（`GET /admin/users` を利用）。メールはハッシュ保存で取得できないため、ピッカーは**登録サーバーのキャラクター名**（とユーザーID）で識別・検索する
...
- **項目名候補マスタ（`bonus_value_labels`・editor/admin）**: 登録フォームの入力候補・一覧の絞り込み候補に出す項目名を `/admin/bonus-value-labels` で管理。画面は **「付加効果の項目名」（`kind='bonus'`）／「追加効果の項目名」（`kind='stat'`＝追加効果「その他」の項目名）のタブ切替**で、各タブが **整理済み（左）／未整理（右）の2ペイン** で構成される。未整理カードを左の任意の位置へドラッグして整理する（左内の並べ替え・左→右へ戻すも同じ操作で可能）。手動追加・アイテム登録時の自動追加はいずれも **未整理（右）** に入る。候補は整理済み→未整理の順で表示される。整理（organize）は種別内で完結し、他タブの整理状態に影響しない
  - **統合**: 未整理カードの「統合」ボタンから**整理済みの項目名を統合先に選択**すると、統合元を使用しているアイテム側のデータ（bonus タブ=付加効果の値の項目名 / stat タブ=追加効果「その他」のキー）を統合先の項目名へ一括更新し、統合元の項目名を削除する（`POST /api/admin/bonus-value-labels/:id/merge`）。自動追加で溜まった表記ゆれ（誤字・別表記）を正とする項目名へ寄せるための操作。実行前に確認ダイアログを出し、完了時に更新したアイテム件数を表示する
- **アイテム種別管理（`excluded_items` ほか・admin）**: アイテムボックスの共通の表示種別（ジャンル）割当を `/admin/excluded-items` で管理（改行・カンマ区切りで一括追加・検索・削除）。共通割当リストは**種別タブ＋あいうえお順**で表示し、**チェックボックスで複数選択して一括削除**できる。アイテムボックスではこの割当でアイテムの表示種別が決まる（取引可能＞ユーザー割当＞共通割当＞未設定。ユーザーは共通種別を自分用に上書き可）。**ユーザーが個別に設定した種別（DB保存分）を集計表示**し（共通化セクション・候補が無くても常時表示）、各候補に**ユーザーが設定した種別の内訳**（種別ごとの人数）を表示する。件数の多い順に種別を選んで**共通化（昇格）**できる（`GET /api/admin/excluded-items/user-suggestions`）。**既に共通登録済みのアイテムをユーザーが別種別へ変更した場合も「上書き候補」として表示**し（「現在: ◯◯」付き）、共通化すると現在の共通種別を上書き更新する。あわせて**サーバ登録対象外（`server_excluded_items`）**もこの画面で管理する
...
- **見出し行**: 1行目は `MoE Trade（6/12）📢`。続けて期間モードのみ【期間中の取引成立数】n件、
  さらに【現在の登録数】出品n件:買取n件（集計期間に依存せず、現在有効な出品・買取の総数。公開一覧と同じ active かつ非凍結ユーザー条件）
  - **📢は行頭に置かない**（日付の後ろに付ける）: Xの投稿画面（Web Intent）は本文の先頭がサロゲートペアの絵文字だと
    その前に空行を1行差し込むため（絵文字の無い2通目以降では起きない）。文面の先頭は必ずASCIIから始める
- **文面フォーマット**: 通常（非オークション）の出品・買取は【新規の取引】見出しの下に1つのリストとして「売)アイテム名 価格」「買)アイテム名 価格」の行で並べる
  （出品→買取の順）。両方0件のときは「新着の出品・買取はなし」。見出しがツイート末尾に孤立しないよう調整
  - **交渉可（`trade_type = negotiable`）は価格の後ろに「:交渉可」を付ける**（例: `売)剛力の剣 12,000AC:交渉可`）。即決は付けない
- **【オークション現在価格】セクション**: オークション（`trade_type = auction`）は【新規の取引】ではなく専用セクションに、
  **現在価格（最良入札 or 開始価格。`App\Support\Auction::currentPrice`）** で掲載する（出品→買取の順・「×N」集約も同様）。
  対象は期間内に新着扱い（`COALESCE(bumped_at, created_at)`。出品時＋入札による価格更新時）になった **進行中（active）のみ**
  （終了済み completed/expired に「現在価格」は無いため除外）。対象0件のときはセクションごと省略する。
  件数はAPIレスポンスの `auction_count`（出品＋買取の合計）で返し、管理画面の統計行に表示する
- ハッシュタグは `#MasterofEpic #MoETrade`（`PromoTweetComposer::HASHTAGS`）。複数ツイートに分割された場合、ゲーム名タグ `#MasterofEpic`（`GAME_HASHTAG`）は1通目のみに付け、共通タグ `#MoETrade`（`COMMON_HASHTAGS`）は全ツイートに付ける（返信スレッドでのゲーム名タグの重複を避ける）
- **投稿はX APIを使わない**（無料枠が2026年2月に廃止されたため）。各文面の「Xでポスト」ボタンが
  ...
  - **スマホ（iOS / Android）でもWeb Intentをそのまま開く**: Xアプリがインストールされていれば x.com のリンクをユニバーサルリンク／アプリリンクで引き受け、アプリの投稿画面に文面が反映される（無ければブラウザの投稿画面）。旧ディープリンク `twitter://post?message=...` は現行のXアプリで本文が反映されないため廃止（UA判定・フォールバックタイマーも撤去）
- **文字数制限と自動分割**: 未認証アカウントの上限（重み280＝全角140字相当。CJK=2・半角英数=1・URL=23換算）に収まるよう、
  アイテムが多い場合は複数ツイートへ自動分割し**全アイテムを漏れなく掲載**する
  - 2通目以降は**1通目への返信として投稿する前提**（スレッド化）。サイトURL（`frontend_url` ＋ `/all`＝出品・買取をまとめて見られる「全て」タブ）とゲーム名タグ `#MasterofEpic` は1通目にのみ付け、共通タグ `#MoETrade` は全ツイートに付ける
...
- 集計仕様: 出品・買取は対象期間（JST）に作成されたもの（取り下げ済み `cancelled` は除外）。
  ...
  同一アイテム・同一価格・同一取引方法は「×N」に集約（同条件の複数出品をまとめる。即決と交渉可は別行）。取引件数は `trade_history` の有効分（`is_valid = true`）のみ
...
- **MoE公式素材の扱い（重要）**: `public/img/` のうち **公式配布バナー（`banner/`・`side_banner/`）・ファンサイトキット由来のイラスト（`castle.jpg`）・ゲーム内アイコン（`favicon.jpg`）と、その派生物である PWA アイコン（`icon-192.png`・`icon-512.png`・`icon-maskable-512.png`）は MoE ファンサイトガイドラインにより「個人の非営利WEBサイトでのみ使用可」のため、公開リポジトリに含めない**（`.gitignore` 済み。正本は非公開の運用置き場で管理し、ローカル・本番には実ファイルを配置する）。自作素材（`logo*.png`・`site_ogp.jpg`・`tours/`）は追跡してよい
- **ヘッダーロゴ**: 画像ロゴ `public/img/logo_header.png`（高さ40px・クリックでトップへ）。元画像 `public/img/logo.png`（透過PNG）からの縮小版で、差し替え時は縮小版を再生成する
- **ヘッダーのナビ構成**: 並び順は `マイ取引`（`/mypage`・ログイン時のみ）→ `アイテムボックス`（`/mypage/items`・ログイン時のみ）→ `出品一覧` → `買取一覧` → `アイテム一覧`（`/items`・全ユーザー。`unverifiedItemCount` のバッジ付き）→ `お問い合わせ`（ログイン時のみ）→ `管理`（editor/admin のみ）。デスクトップ・モバイルドロワーとも同一順。マイページ階層・アイテム一覧は以前ドロップダウン配下だったがトップ階層へ移動済み
- **ヘッダー右側の操作**: デスクトップ（lg以上）はナビの右側に、ログイン時は「⚙️ 通知設定」（`/mypage/notifications`。マイページのヘッダー行から移動）→「ログアウト」、未ログイン時は「ログイン」「新規登録」を表示する。モバイルドロワーでは最下部（ログアウトの上）に「⚙️ 通知設定」を置く
- **ヘッダー「管理」ドロップダウン**（デスクトップナビ・lg以上、editor/admin のみ表示）: 「管理」ボタンで開閉トグル。開いている間はメニュー外での `mousedown` で自動的に閉じる（メニュー内・ボタン上のクリックでは閉じない）。中身は editor/admin の項目名・バインダー管理と admin 限定のユーザー/お知らせ/宣伝/アイテム種別管理（アイテム一覧はトップ階層へ移動したため非掲載）
...
- **モーダルの描画位置（暗幕とヘッダー）**: 全画面オーバーレイ（`fixed inset-0` の暗幕）を持つモーダルは、ページ本文コンテナの `space-y-*`（隣接兄弟に `margin-top` を付与）の直下に置くと、`fixed` 要素が margin ぶん下方向にずれて上端（ヘッダーメニュー）が暗幕で覆われない不具合が出る。対策は2通り:
  - **共通モーダルコンポーネント**は **`react-dom` の `createPortal` で `document.body` 直下に描画**して親レイアウトから切り離す（`EditTradeModal` / `RenewTradeModal` / `PriceAnalyticsModal` / `CandidateSelectModal`）。回帰テスト `frontend/src/components/RenewTradeModal.test.tsx`
  - **ページ内に直書きしたオーバーレイ**は暗幕 div に `!mt-0` を付けて `space-y` のマージンを打ち消す（`OwnedItemsPage` / `admin/AdminItemsPage`）
...
- **フッターバナー**: 768px（md）以上で `public/img/banner/` の5種からランダムに1つをフッター右端に表示し、公式サイトへ新規タブでリンク。著作権表記はバナーを除いた残り幅の中で中央寄せ
- **MoE公式ガイドライン対応（フッター）**: ガイドライン指定の著作権表記（`(C)MOE K.K. (C)Konami Digital Entertainment …`）をフッターに常時表示し、公式サイトへのリンクは必ずトップページ **`https://moepic.com/`** に向ける（`Footer.tsx` / `SideBanners.tsx`）。回帰防止テスト: `frontend/src/components/Footer.test.tsx`
...
- **配布用バナー（他サイト掲載用）**: MoE公式サイト等に当サイトを掲載してもらう際に提供するバナー。`public/img/site_banner_200x40.png`（PNG・7,981B）と同 `.gif`（4,616B）の2形式を配置する。サイズは **200×40px**、容量 **10KB以下**（MoE公式サイトのバナー規定）。ヘッダーロゴ `logo_header.png` を高さ36pxに縮小して左に置き、右に「アイテム取引所／出品・買取・相場」を添えたダーク×金の意匠。**自作素材のみで構成**しているため公開リポジトリに追跡してよい（公式配布バナー `banner/`・`side_banner/` とは別物。上記「MoE公式素材の扱い」参照）。生成元スクリプトは持たず、差し替え時はこのファイル自体を直接更新する
...
- **出品一覧・買取一覧の表示モード（詳細 / シンプル）**: 一覧の情報量をユーザーが自分で選べるようにする。ソート選択の左に「詳細 / シンプル」のトグル（`frontend/src/components/ListDisplayToggle.tsx`・`role="group"` / 各ボタン `aria-pressed`）を置き、**シンプル表示ではアイテム名・取引可能サーバー・価格・ボタン（取引／相場情報）だけ**を表示する。シンプル表示で落とすもの: 効果列（追加効果/付加効果/特殊条件・情報・必要スキル/マスタリ・設置/ストレージ）、種別（カテゴリ名）・装備セット/削れ/染色/確認中バッジ・公式DBリンク・ハッシュタグ・セット部位名、取引方法バッジ、残り期限・入札件数/即決価格、出品/買取コメント行。**サーバー列はシンプル表示では常に表示**する（列を絞っており幅に余裕があるため、コンテナクエリの `listing-col-wide` は付けない）。操作列も同じ理由で幅によらず「相場情報」ボタンを出す（「詳細 →」リンクは詳細表示のみ）。**ただしスマホ（768px未満）ではサーバー列・操作列も落として「アイテム名・価格」の2列だけ**にする（下記）
  ...
  - **スマホ（768px未満）のシンプル表示は「アイテム名・価格」の2列だけ**にする: サーバー列（`w-px whitespace-nowrap`）と操作列（`w-20` のボタン）はどちらも最小幅を持つため、スマホ幅ではアイテム列に残り幅がほとんど無くなり**アイテム名が1〜2文字ずつ折り返されて読めなくなる**。そこでシンプル表示かつ狭い端末（`useMediaQuery('(min-width: 768px)')` が偽 = `compactMobile`）では**サーバー列・操作列（取引／相場情報ボタン）を描画しない**。左右パディングも詰め（アイテム `px-3` / 価格 `px-2`）、価格列は `w-px whitespace-nowrap` にして残り幅を全部アイテム名に寄せる。**列の出し分けは CSS ではなく JSX で片方だけ描画**する（コンテナクエリだと DOM に残ったセルの最小幅が効いてしまうため）
  - **スマホのシンプル表示は行タップで取引詳細へ遷移**: 操作列（「取引」「相場情報」）を落としている代わりに、行のどこをタップしても出品詳細 `/listings/:id`・買取詳細 `/buy-requests/:id` へ移動する（`<tr>` の `onClick` ＋ `cursor-pointer`）。アイテム名自体もキーボード操作・リンク共有のため同じ URL の `<Link>`（`block`）にし、行の `onClick` と二重に発火しないよう `stopPropagation` する。**スマホのシンプル表示ではアイテム名からアイテム恒久ページ `/items/:id` へは飛ばさない**（一覧で見たい情報は取引そのものなので取引詳細を優先。アイテム詳細は取引詳細ページから辿れる）
  ...
  - **シンプル表示のアイテム名クリックでアイテム詳細を開く（PC のみ）**: 種別・効果・公式DBリンクを落としている代わりに、アイテム名から詳細へ辿れるようにする。PC（`useMediaQuery('(min-width: 768px)')` が真）は共通の `ItemDetailModal` をポップアップ表示する（スマホでは縦に長い詳細モーダルが扱いづらいため使わず、上記のとおり取引詳細へのリンクにする）。**出し分けは CSS ではなく片方だけ描画**する（両方描画するとアイテム名が DOM に二重に現れるため）。詳細表示のアイテム名は従来どおりただのテキスト
  ...
  - ページツアーの対象要素（`data-tour="listings-tradetype"` 等）はシンプル表示では存在しないが、`TourOverlay` が「対象が見つからない／非表示のステップは自動スキップ」するためツアーは破綻しない
...
- **詳細ページのサーバ側メタ注入（インデックス対策の本丸）**: SPA の `index.html` は全URLで同一の汎用 `<title>`/description しか持たないため、
  クローラーは JS レンダリング前の生HTMLでは個々のページを区別できず「クロール済み・未登録（Crawled - currently not indexed）」「重複」「ソフト404」に滞留しやすい。
  そこで**本番では `/items/{id}`・`/listings/{id}`・`/buy-requests/{id}`（いずれも数値ID）だけ Laravel が受け**、ビルド済み `index.html`（`backend/public/index.html`）に
  ページ固有のメタを流し込んでから返す。共通の注入処理は `SpaPageController`（基底クラス）に集約し、各コントローラが継承する（`routes/web.php`）:
  - `ItemPageController`（`/items/{id}`）: `<title>`/description/OGP（`og:title`/`og:description`/`og:url`・`twitter:*`）/`canonical`=自URL/Product JSON-LD（未確認は `noindex`）。存在しなければ 404＋SPAシェル。
  - `ListingPageController`（`/listings/{id}`）・`BuyRequestPageController`（`/buy-requests/{id}`）: `<title>`（「`<アイテム名>` の出品/買取」）/description/OGP（`og:url`=自URL）と、
    **`canonical`=アイテム恒久ページ `/items/{item_id}`**（生HTMLの段階で canonical 集約が伝わり、Search Console の「重複しています。ユーザーにより、正規ページとして選択されていません」を防ぐ）。
    閲覧可否は詳細API（`show`）と同じ `visible(['active','completed'])` をミラーし、**取り下げ・期限切れ（バッチ未実行の active 期限超過含む）・不存在は本物の 404** を返す
    （中身のない 200 を返して Search Console で「ソフト404」になるのを防ぐ）。
  アイテム名はユーザー投稿（未確認）由来があり得るため、`htmlspecialchars` と `json_encode(... JSON_HEX_TAG)` でエスケープして注入する。
  ...
  404 でも SPA シェルは返す（クライアント側ルーターが not-found 表示）。
  （開発 nginx はこれらを Vite にプロキシしたままで、生HTMLメタ注入は本番のみ。Googlebot は開発環境をクロールしないため問題ない）
...
- **ルーティング**: `/sitemap.xml` と `/items/{数値ID}`・`/listings/{数値ID}`・`/buy-requests/{数値ID}` は本番 `.htaccess`（非公開運用ファイルで管理）で Laravel に振り分ける（SPAフォールバックさせない）。
...
- 回帰防止テスト: `backend/tests/Feature/SitemapTest.php` / `backend/tests/Feature/ItemPageTest.php` / `backend/tests/Feature/ListingPageTest.php` / `backend/tests/Feature/BuyRequestPageTest.php` / `backend/tests/Unit/RobotsTxtTest.php` / `backend/tests/Unit/GoogleSiteVerificationTest.php` / `frontend/src/hooks/usePageMeta.test.tsx` / `frontend/src/pages/ItemDetailPage.test.tsx`
...
- **マニフェスト**（`frontend/public/manifest.webmanifest`）:
  ...
  **3点ともファビコン `img/favicon.jpg`（44×44）を拡大して生成する**（ブラウザのタブとホーム画面で同じ絵柄に揃えるため）。`any` の2枚は favicon を全面に拡大したもの、`maskable` は中央80%のセーフゾーンに収まるよう 300×300 に縮めて背景 `#111827` の 512×512 キャンバス中央へ置いたもの。元画像が 44×44 しかないため拡大で甘くなるのは承知の上（より大きな原本が手に入ったら再生成する）。差し替えは favicon から同じ手順で作り直す。
...
- **インストールボタン**（`components/InstallAppButton.tsx` / `utils/installPrompt.ts`）:
  ...
  `prompt()` は使い切りのため実行後はバナーを消す。「あとで」は localStorage `pwa_install_dismissed='1'` で以後非表示（判定・記録は `utils/installPrompt.ts` の `isInstallDismissed()` / `markInstallDismissed()` に集約）。
  ...
  **「あとで」でバナーを消した端末向けの再導線**として、通知設定画面（`/mypage/notifications`）の先頭に「アプリをインストール」ボタンを表示する（インストール可能かつ「あとで」済みかつ非 standalone のときのみ。§8「通知設定」参照）。
...
├── /others                   # 出品一覧・検索（その他タブ：未開封ペット・レシピ・ペット用アイテム・アイテムセット・選べるチケット）
│   └── /listings/:id         # 出品詳細（価格解析・取引希望送信。チャットのやり取りはマイページ）
├── /listings/new             # 出品登録フォーム（登録完了後は元居た画面へ戻る。直リンク等で戻り先が無ければ /mypage）
...
├── /privacy                  # プライバシーポリシー（公開ページ・個人情報保護法の公表事項）
├── /admin → /items
...
- 各部位は**通常アイテムとして独立登録**され（部位ごとに名前・公式DB・追加効果・付加効果・特殊条件などを保持）、
...
- 構成部位として選択できるカテゴリは装備部位（武器・防具・装飾品）と**テクニック（ノアピース・秘伝の書）**。
  装備部位になり得ない「装備セット」「その他（未開封ペット・レシピ・ペット用アイテム・アイテムセット・選べるチケット）」は部位候補チェックボックスに表示しない。
...
- **テクニック部位の必要スキル値・必要マスタリ**: テクニック部位は効果の代わりに、部位ごとの
  「必要スキル・マスタリ」セクション（折りたたみ）で必要スキル値と必要マスタリ（複数はOR条件）を入力する。
  送信時は空文字スキルを除いて数値化した `pieces.*.skill_requirements` / `pieces.*.mastery_requirements` を送り
  （未設定は null。装備部位は常に null を送って既存値もクリア）、部位アイテムの同名カラムへ保存する。
  マスタリコードは `App\Support\Mastery::codes()` のホワイトリストで検証する。
  詳細のセット内訳（`EquipmentSetBreakdown`）ではテクニック部位の必要スキル・必要マスタリをバッジ表示し、
  性能グルーピングキー（`groupPiecesByPerformance`）にも必要スキル・マスタリを含める（異なるテクニック同士を誤って統合しない）。
- 編集画面で種別を **部位（装備品）↔ 装備セット** で切り替えたとき、部位フォームの追加効果・付加効果・特殊条件と
  装備セットの「全部位共通」グループ[0]の間でデータを引き継ぐ（誤操作・再検討で入力を失わないため）。
  追加した設定グループ[1..]や構成部位（parts）はそのまま保持する。
...
- 一覧の**付加効果列**では、テクニック部位（ノアピース・秘伝の書）は付加効果を持たないため効果グループに含めず、
  **部位カテゴリ名チップ＋部位アイテム名**で表示する（`SetBonusCell` に `categories` を渡してテクニック判定。
  未指定時は従来どおり全部位を効果でグループ化するフォールバック）。
- テクニックは**特殊条件も設定しない**。一覧の**追加効果列・特殊条件列**でもテクニック部位はグループ化対象から外し、
  「ノアピース: —」のような空グループを表示しない（`SetBaseStatsCell` / `SetSpecialConditionsCell` も `categories` で判定。
  テクニック部位のみのセットはダッシュ表示）。テクニック部位のアイテム名表示は付加効果列に集約する。
- **「すべて」タブの情報列**では、テクニック部位を付加効果内に混ぜず（`SetBonusCell` の `showTechniqueNames=false`）、
  情報列の**最後に「テクニック」枠**（部位カテゴリ名チップ＋部位アイテム名。`TechniquePieceNames`）として表示する
  （表示順: 追加効果 → 付加効果 → 特殊条件 → テクニック）。
...
アセット種別のアイテムは追加効果・付加効果・染色・ミスリル・必要スキル値を持たず、代わりにアセット固有のパラメータ（設置個所・サイズ・ストレージ数・特殊機能）を設定する。特殊条件は装備品と共通で利用できる。
出品一覧・管理画面では「アセット」タブで表示が切り替わり、効果列が「設置・サイズ」「ストレージ・特殊機能」「特殊条件」表示になる。

### その他
既存種別に当てはまらないアイテムの受け皿。最上位カテゴリ「その他」の子カテゴリ「未開封ペット」「レシピ」「ペット用アイテム」「アイテムセット」「選べるチケット」を種別として選択する。
登録・編集フォームで「その他」配下を選ぶと「適切な種別がない場合、お問い合わせでご連絡おねがいします！／時の石など、Ancient Age で取引可能な種別は対象外とさせていただいています！」の案内を表示する。
...
- **選べるチケット**: 引き換え時に**いずれか1つを選べる**アイテムの候補リストを入力する。データ構造・入力 UI・保存時の正規化はアイテムセットと**完全に共通**（同じ `set_items` カラム・同じ `SetItemsEditor`）で、種別（カテゴリ名）だけが異なる。表示ラベルのみ「アイテムリスト」ではなく **「選べるアイテム」**（`itemType.ts` の `setItemsLabel()`）とし、フォームの見出しは「選べるチケット情報」になる。`set_items` を持つ種別かどうかの判定は `itemType.ts` の `hasSetItems()` に集約する。
  - **アイテム名検索の対象**: 出品一覧（`GET /api/listings` の `item_name`）・買取一覧（`GET /api/buy-requests` の `item_name`）・アイテム一覧（`GET /api/items` の `name`）の名前検索は、アイテム名に加えて **アイテムセット／選べるチケットの `set_items` 内の名前にも部分一致**する（`CAST(set_items AS CHAR) LIKE` の OR 条件。買取の複数アイテム名 `item_names[]`＝アイテムボックス貼り付けの完全一致検索は対象外）。このため `set_items` は Unicode エスケープなしで保存する（`App\Casts\UnescapedJson` キャスト。標準 'array' キャストは日本語を `\uXXXX` にエスケープし、SQLite テストで LIKE が一致しないため）。
- **レシピ**: **レシピ名・必要スキル値の組（エントリ）を複数登録できる**（`items.recipe_entries` JSON 配列。要素は `{ "name", "skill_requirements" }`）。装備セット（1本体に複数の設定グループ）を参考にした構造で、**必要スキル値はレシピ名ごとに個別**に設定する（作成に必要なスキル値。必要マスタリは持たない）。レシピはバインダーを持たない（旧仕様のバインダー入力は廃止）。
  ...
  - 必要スキル値の入力欄は共通の `SkillRequirementInputs` で **戦闘 / 基本 / 生産 / 熟練 のグループ単位のアコーディオン**表示。**レシピは「生産」グループのみ初期展開**（テクニックの必要スキル値は全グループ初期展開）。

その他種別のアイテムは追加効果・付加効果・染色・ミスリルを持たない（必要スキル値はレシピのみ保持。特殊条件は持たない）。
出品一覧・管理画面では「その他」タブ（`item_type=other`）で表示が切り替わり、効果列が「情報」列（ペット名／レシピの各エントリ＝レシピ名・そのレシピ名の必要スキル値／ペット用アイテムの対象ペット・効果／アイテムセット・選べるチケットのアイテムリスト）になる。アイテムリストは**1つの枠内に1行1アイテムで改行表示**する（一覧の情報列 `OtherInfoCell`・アイテム詳細 `ItemInfoCard` とも同じ表示）。買取一覧は種別タブを持たないためアイテム名で横断検索する。
出品時は耐久度・染色の概念がないため「削れあり」「染色済み」の入力を表示しない（テクニックと同様。`NewListingPage` で最上位カテゴリが「その他」のとき非表示）。

### 公式DB（公式サイトリンク）
全種別共通の任意項目。MasterOfEpic公式サイト（`moepic.com`）のアイテムページへのリンク（`items.official_url`）を登録・編集フォーム（`NewItemForm` / `AdminItemEditPage` の基本情報欄）で入力できる。wiki型のため**全ログインユーザー**が設定・編集できる。
- **装備セットは部位ごとにも入力**: 装備セットの各構成部位は独立したアイテムなので公式DBページも部位ごとに異なる。構成部位エディタ（`EquipmentSetPiecesEditor` の「構成部位と名称」欄）で部位名の下に公式DB入力欄を用意し、`pieces.*.official_url` として各部位アイテムの `official_url` に保存する。検証はセット本体と同じ `officialUrlRule`（`moepic.com` 限定）。セット本体の基本情報欄の公式DBはセット全体のリンクとして別途保持できる。
- **入力検証**: バックエンド（`ItemController::officialUrlRule`）で `url` 形式かつホストが `moepic.com`（サブドメイン含む `*.moepic.com`）の場合のみ許可する。それ以外は422で拒否（誰でも編集できる項目のため、外部サイトへの誘導・フィッシング悪用を防ぐ）。
- **`javascript:Move(...)` リンクの自動変換**: 公式サイトのページ内リンクは `<a href="javascript:Move('URL','hidden_key')">` 形式（サイト側 `formcontrol.js` の `Move()` がフォームに `hidden_key` をセットして POST する）。同じページは GET の `?hidden_key=...` でも開けるため、ユーザーが「リンクをコピー」で貼り付けた `javascript:Move('URL','KEY')` は **`URL?hidden_key=KEY` という通常のURLへ自動変換して保存**する（フロント: `utils/officialUrl.ts` の `normalizeOfficialUrl` を各入力欄の onChange に適用して即時変換。バックエンド: `App\Support\OfficialUrl::normalize` を `ItemController::normalizeOfficialUrlInput` が `official_url` / `pieces.*.official_url` にバリデーション前適用）。変換後も `officialUrlRule` のホスト検証が効くため `moepic.com` 以外の URL を含む `Move()` は拒否される。`javascript:` URL 自体は保存しない（XSS防止）。`Move()` の第1引数は絶対URLのほか、**ルート相対パス**（例: `'/top/news_detail.php'`。公式DBは moepic.com 限定のため `https://moepic.com` のオリジンで解決）と**プロトコル相対**（`//host/...` → `https:` を補完）も変換できる。ディレクトリ相対（例: `'news_detail.php'`）のみ元ページが特定できないため変換せず、通常のバリデーションエラーになる。
- **表示**: 設定済みの場合、共通リンク部品 `OfficialDbLink`（`📖 公式DB ↗`）を**出品一覧（`ListingsPage` のアイテム名欄）・買取一覧（`BuyRequestCard`）・アイテムボックス（`OwnedItemsPage` の登録アイテムに紐づく行のアイテム名欄）・アイテム管理一覧（`AdminItemsPage` のアイテム名欄「詳細を見る」の横）・各詳細（`ItemInfoCard` 経由で出品詳細／買取詳細／アイテム詳細）**に表示し、クリックで**新しいウィンドウ**に開く。アイテムボックスでは `POST /api/items/match` で紐づいた登録アイテム（`official_url` を含む）を持つ行にのみ表示する。リンクは `<a>` の入れ子（Link カード内）を避けるため `button` + `window.open(..., '_blank', 'noopener,noreferrer')` で実装する。
...
- **一覧でのテキストの省略**: 一覧（出品一覧・買取一覧・アイテム管理）の追加効果セル（`BaseStatBadges` /
  装備セットの `SetBaseStatsCell`）では、**付加効果のテキストと同じしきい値・同じ表示**で省略する。
  すなわちテキスト値が **5文字以上**（`TEXT_DETAIL_THRESHOLD`）なら値を展開せず「詳細」バッジ（青系）にし、
  ホバー（フォーカス）で全文をポップアップ表示する。4文字以下と数値（桁数に関わらず）はそのまま表示する。
  「全て」タブの情報列は幅広のためしきい値を **20文字** に緩める（`longTextThreshold` prop。付加効果と共通の
  `ALL_INFO_TEXT_THRESHOLD`）。バッジ／ポップアップの実装は付加効果と共通部品（`DetailTextPopover`）。
  アイテム詳細（`ItemInfoCard`）は省略せず全文を表示する。
...
- **テキスト**: 値欄が数値入力からテキスト入力に変わり、文字列をそのまま保持・表示する。
  ただし一覧（出品一覧・買取一覧・アイテム管理・装備セット内訳）の付加効果セル（`BonusEffectList`）では、
  テキスト値が **5文字以上**（`TEXT_DETAIL_THRESHOLD`）の場合は値を直接展開せず「詳細」バッジ（青系）で表示し、ホバー（フォーカス）で全文をポップアップ表示する（長文でレイアウトが崩れるのを防ぐ）。4文字以下はそのまま表示。
  ただし出品一覧の**「全て」タブの情報列**は3列ぶち抜き（colSpan=3）で各種別タブの効果列より幅広のため、しきい値を **20文字** に緩める（`BonusEffectList` / `SetBonusCell` の `longTextThreshold` prop）。列に収まる長さ（19文字以下）のテキストはそのまま表示し、20文字以上のみ「詳細」バッジに切り替える。
  この省略表示は**追加効果「その他」のテキストと共通**（しきい値・バッジ・ポップアップの部品 `DetailTextPopover` を共有）。
...
### 専用技（廃止）
専用技フラグは**廃止した**。登録・編集フォームの「専用技」チェック、付加効果名の横の「専用技」バッジ、
API の入出力（`bonus_effects[].is_exclusive`）をすべて削除し、マイグレーション
`2026_08_20_000001_drop_is_exclusive_from_item_bonus_effects` で `item_bonus_effects.is_exclusive` 列も削除する
（どの付加効果が専用技だったかの情報は失われる）。
旧クライアントが `is_exclusive` を送ってもバリデーションで無視される。
（さらに旧 `items.exclusive_skill`（アイテム単位フラグ）は
`2026_06_13_000005_unify_exclusive_skill_into_bonus_effects` で先に廃止済み）

### WarAge無効（付加効果単位）
「WarAgeでは効果がない」付加効果は **付加効果ごとのフラグ `item_bonus_effects.no_warage_effect`** で保持する（装備品・装備セット部位とも共通）。
登録・編集フォームでは各付加効果に「WarAge無効」チェックを設け、チェック時は付加効果の説明の末尾に「※WarAgeでは効果がない」を付けて表示する（文言は説明本文には保存しない）。
マイグレーション `2026_06_23_000001_add_no_warage_effect_to_item_bonus_effects` で列を追加し、
既存の説明本文に「※WarAgeでは効果がない」が含まれる付加効果は、その文言を説明から取り除いて `no_warage_effect=true` にバックフィルする。

### 必要スキル値（スキル種別・レシピ）
スキル種別のアイテムは、各スキルの必要値（0〜100）を `items.skill_requirements` に
`{ "スキル名": 値 }` 形式のJSONで保持する。例: `{ "刀剣": 80, "筋力": 50 }`。
レシピは必要スキル値を**エントリ（レシピ名）ごと**に `items.recipe_entries[*].skill_requirements` に保持し、
`items.skill_requirements` にはそれらを**各スキル最大値で集約**した検索用の値を持つ（作成に必要なスキル値を表す）。
...
（テクニックの子: ノアピース・秘伝の書。その他の子: 未開封ペット・レシピ・ペット用アイテム・アイテムセット・選べるチケット。装備セット・アセットは子を持たない特殊カテゴリ）
...
| skill_requirements | JSON | スキル種別の必要スキル値。レシピでは `recipe_entries` 全体を各スキル最大値で集約した検索用の値（例: {"刀剣":80,"筋力":50}）。NULL = 非該当 |
...
| recipe_name | VARCHAR(200) | その他「レシピ」: レシピ名（`recipe_entries` 第1エントリからの派生互換値）。NULL = 非該当 |
| recipe_binder | VARCHAR(100) | レガシー: 旧レシピのバインダー。現在レシピでは未使用で常に NULL（列は残置） |
| recipe_entries | JSON | その他「レシピ」: {name, skill_requirements} エントリの配列（レシピ名・必要スキル値の組を複数）。NULL = 非該当 |
| target_pet | VARCHAR(100) | その他「ペット用アイテム」: 対象ペット（自由入力）。NULL = 非該当 |
| pet_item_effect | VARCHAR(500) | その他「ペット用アイテム」: 効果（自由入力）。NULL = 非該当 |
| set_items | JSON | その他「アイテムセット」: 構成アイテム名 / その他「選べるチケット」: 選べるアイテム名の文字列配列（自由入力・複数）。保存時に空エントリを除去し全て空なら NULL |
...
| description | TEXT | 説明文 |
| no_warage_effect | BOOLEAN | 「WarAgeでは効果がない」フラグ（デフォルト: false）。true のとき説明の末尾に「※WarAgeでは効果がない」を付けて表示する（文言は説明本文には保存しない）。付加効果単位で保持 |
...
| trade_type | ENUM('fixed','negotiable','auction') | 取引方法（即決 / 交渉可 / オークション）。`auction` では `price` = 最低取引価格（開始価格 兼 下限）、`expires_at` = 期限日 |
| buyout_price | INT NULL | 即決価格（オークションのみ・任意）。入札がこの額**以上**で即時成立。非オークションは NULL |
| comment | TEXT | 出品コメント |
...
| bid_price | INT NULL | 入札額（オークションの取引希望＝入札のとき設定。非オークションは NULL）。出品=高いほど有利 / 買取=安いほど有利 |
| outbid_at | TIMESTAMP NULL | より有利な入札に抜かれた時刻（他の入札者への価格更新通知に使用。現在の最良入札は NULL） |
...
- **オークションの入札（`bid_price` あり）は先着順キューを適用しない**。全入札が価格で並列に競い、owner からも匿名化されず全件見える（前述「オークション」参照）
...
| trade_type | ENUM('fixed','negotiable','auction') | 取引方法（即決 / 交渉可 / オークション）。`auction` では `price` = 最高取引価格（開始価格 兼 上限） |
| buyout_price | INT NULL | 即決価格（オークションのみ・任意）。入札がこの額**以下**で即時成立。非オークションは NULL |
| comment | TEXT | コメント |
...
| target_type | VARCHAR(20) | 表示対象（all=全員・デフォルト / staff=管理・編集者のみ / specific=指定ユーザーのみ） |
| target_user_ids | JSON | target_type=specific のときの対象ユーザーID配列（それ以外は NULL） |
...
|---|---|---|
| id | BIGINT PK | |
| command | VARCHAR(100) | 実行したコマンド名（例: listings:expire。index 付き） |
| status | VARCHAR(20) | running（実行中）/ success（正常終了）/ failed（例外発生・デフォルト running） |
| summary | TEXT | 実行結果の要約（成功時）または例外メッセージ（失敗時） |
| started_at | TIMESTAMP | 開始日時 |
| finished_at | TIMESTAMP | 終了日時（NULL = 実行中/異常終了） |
| duration_ms | INT UNSIGNED | 所要時間（ミリ秒） |
| created_at / updated_at | TIMESTAMP | |

※ `(command, started_at)` に複合 index。記録は `App\Console\Commands\BatchCommand` が自動で行う。

### user_daily_accesses（日＋時間帯ごとのユニークアクセスユーザー記録）
| カラム | 型 | 説明 |
|---|---|---|
| id | BIGINT PK | |
| user_id | BIGINT FK | users.id（cascade delete） |
| date | DATE | アクセス日（**JST・Asia/Tokyo の日付**で保存。集計時のタイムゾーン変換を不要にする） |
| hour | TINYINT UNSIGNED NULL | アクセスした**時（JST の 0〜23）**。`null` は時間帯の記録を始める前に作られた古い行（時間帯分布からは除外する） |

※ `(user_id, date, hour)` に UNIQUE、`date` に index。timestamps なし。`RecordDailyAccess` ミドルウェア（api グループ）が認証済みリクエストごとに `insertOrIgnore` で記録し（同じ日・同じ時間帯の2回目以降はユニーク制約で no-op）、利用状況解析の「アクセス」系列の集計元になる。日次ユニークアクセスユーザー数は `date` ごとに `user_id` を重複排除して数え、時間帯分布は `hour` ごとの行数（＝日ごと・時間帯ごとのユニークユーザーの**延べ数**）で求める。`user_id` の外部キーはこのユニーク索引を使うため、索引を張り替えるマイグレーションは**新ユニークを作ってから旧ユニークを落とす**順序にする。

### bonus_value_labels（項目名候補マスタ・付加効果/追加効果）
| カラム | 型 | 説明 |
|---|---|---|
| id | BIGINT PK | |
| kind | VARCHAR(20) | 種別。`bonus`=付加効果の項目名（default） / `stat`=追加効果「その他」の項目名 |
| label | VARCHAR(100) | 項目名（例: 物理ダメージ）。`(kind, label)` で UNIQUE（同名でも種別が違えば別候補） |
...
- アイテム登録フォームの項目名候補入力（`ComboInput` の自前ドロップダウン。IME 変換中の連結不具合を避けるためネイティブ datalist は不使用）・一覧の絞り込み候補に使用。アイテム登録時に未登録の項目名は自動追加（付加効果 `BonusValueLabel::syncFromBonusEffects()` → `kind='bonus'`、追加効果「その他」 `syncFromBaseStats()` → `kind='stat'`。自動追加分は **未整理** で入る）
- 公開候補 API は **整理済み（sort_order 順）→ 未整理（文字順）** の順で返す（`?kind=` で種別指定・未指定は `bonus`）

### binder_labels（バインダー候補マスタ・レガシー）
| カラム | 型 | 説明 |
|---|---|---|
| id | BIGINT PK | |
...
- 旧「レシピ」のバインダー入力の候補に使用していたマスタ。**レシピからバインダーを廃止したため現在は未使用**（テーブル・管理画面 `/admin/binder-labels`・公開候補 API は残置するが、レシピ登録からの自動追加・参照は行わない）。`BinderLabel::syncFromBinder()` も呼び出し元なし。
...
- 公式サイトのアイテムボックスはアカウント単位のため、所持品整理用のアカウント名（複数可）。取引用キャラクター `user_characters` とは別概念。名前は台帳を分けるための管理用ラベルで、実際のアカウント名と一致している必要はない
...
### exclusion_types（表示種別・admin管理）
| カラム | 型 | 説明 |
|---|---|---|
| id | BIGINT PK | |
| name | VARCHAR(100) UNIQUE | 種別名 |
| is_default | BOOLEAN | 既定種別かどうか（既定は「その他」1件のみ true） |
| default_enabled | BOOLEAN | 旧仕様（除外の既定適用）の名残り。**表示種別方式では未使用**（既定表示は「取引可能」のみ）。列・API は後方互換で残置 |
| sort_order | INT | 表示順 |
| created_at / updated_at | TIMESTAMP | |

- アイテムを分類する表示種別（ジャンル）。**既定種別「その他」**（`is_default=true`）はマイグレーションで投入し、**削除できない**（改名は可）
- 管理画面「アイテム種別管理」の種別の改名は、ブラウザ標準の `window.prompt` ではなく**共通ダイアログ（`useDialog().prompt`）のテキストボックス**で入力を受け取る（ネイティブ prompt は環境により抑制され改名ボタンが無反応になるため。他画面の入力ダイアログと同方式）
- 種別を削除すると、その種別に属する `excluded_items` は既定種別へ付け替えてから削除する（割当自体は消えない）
- ユーザーはアイテムボックスの種別タブで表示する種別を切り替える（既定は「取引可能」のみ）。行の実効種別は `effectiveTypeId`（`frontend/src/utils/inventory.ts`）で算出

### excluded_items（共通の種別割当・admin管理）
| カラム | 型 | 説明 |
|---|---|---|
| id | BIGINT PK | |
| name | VARCHAR(200) UNIQUE | アイテム名（文字列・完全一致で判定） |
...
- name→種別の**共通割当**（管理者）。アイテムボックスで未登録アイテムの表示種別を決める。

### user_excluded_items（ユーザー個別の種別割当・DB保存時）
| カラム | 型 | 説明 |
|---|---|---|
| id | BIGINT PK | |
| user_id | BIGINT FK→users | cascade |
| name | VARCHAR(200) | アイテム名（`unique(user_id, name)`） |
| exclusion_type_id | BIGINT FK→exclusion_types NULL | nullOnDelete。種別（null は既定種別「その他」）。旧「個別除外」の既存行は null のまま＝その他に集約 |
| user_exclusion_type_id | BIGINT FK→user_exclusion_types NULL | nullOnDelete。ユーザーのカスタム種別への割当（設定時は exclusion_type_id より優先。このとき exclusion_type_id は null で保存） |
| created_at / updated_at | TIMESTAMP | |

- name→種別の**ユーザー割当**。ユーザー割当は共通割当（`excluded_items`）より**優先**される（ユーザーが共通種別を自分用に上書きできる）。共通と**種別が異なる**割当（＝上書き）はスナップショットでそのまま返すが、**共通と同じ種別の割当は冗長なので保存しない・スナップショットからも除外する**（共通に従う。`null` は既定種別へ正規化して比較）。共通へ昇格した名前は昇格時に `user_excluded_items` から削除されるため通常は重複しない。
- **カスタム種別への割当（`user_exclusion_type_id` 付き）は常にユーザー固有**なので、共通との冗長判定の対象外（保存・スナップショットとも常に返す）。共通種別への昇格（共通化）時も**削除されない**（本人の明示的な分類として残り、共通より優先のまま）。管理画面の共通化候補（`user-suggestions`）の人数集計・種別内訳からも除外する（`exclusion_type_id=null` のカスタム割当を「その他」に誤計上しない）。

### user_exclusion_types（ユーザーごとのカスタム表示種別・DB保存時）
| カラム | 型 | 説明 |
|---|---|---|
| id | BIGINT PK | |
| user_id | BIGINT FK→users | cascade |
| name | VARCHAR(100) | 種別名（`unique(user_id, name)`） |
| sort_order | INT | 表示順 |
| created_at / updated_at | TIMESTAMP | |

- 共通種別（`exclusion_types`・admin管理）とは別に、**各ユーザーが自分専用の表示種別**を追加できる（アイテムボックスの「カスタム種別」）。保存先がローカルのユーザーはクライアントの `localStorage` にのみ持つ（このテーブルは DB保存時のみ使用）
- 台帳の全置換 PUT（`/api/mypage/inventory`）で **name 単位に upsert** される（id を保ち、端末に保存した種別タブ選択などの参照が保存のたびに壊れないようにする）。payload に無くなった種別は削除
- 割当は `user_excluded_items.user_exclusion_type_id`。カスタム種別を削除すると割当はクライアント側で一緒に解除される（DB側の FK は nullOnDelete）

### server_excluded_items（サーバ登録対象外・システム共通・admin管理）
| カラム | 型 | 説明 |
|---|---|---|
| id | BIGINT PK | |
| name | VARCHAR(200) UNIQUE | アイテム名（文字列・完全一致で判定） |
...
- ここに登録された名前のアイテムは、保存先がサーバー（DB）でもサーバーへ保存せず各ユーザーの端末（ローカル）にだけ保存する（分割保存）。公開 `GET /api/server-excluded-items`（名前配列）でクライアントが判定に使う。**ユーザー個別指定分はサーバーへ送らず端末ローカルにのみ保持**（キー `moe_inventory_server_excluded`）。
- 既定シード（マイグレーション）: **「エンシェント コイン」**をシステム共通の対象外として投入する。
...
- 保存先が**端末（ローカルストレージ）**のユーザーの種別割当は `user_excluded_items` に入らないため、共通種別への昇格を検討できるよう分類された「名前」だけを匿名で集める。**誰が・何人分類したかは持たない**（`user_id` なし・名前は一意）
- フロントは端末保存時に「種別の割当」操作のたび `POST /api/excluded-items/report` で名前を fire-and-forget 送信する（失敗は致命でない）。送るのは名前のみ・匿名のため、ユーザー向けの追加注記は設けない
- `user-suggestions`（共通種別への昇格候補）に DB保存分とマージして `from_device=true` で表示する。各候補には**ユーザーが設定した種別の内訳** `type_assignments`（`[{type_id, count}]`・多い順・null=その他）と**最も多く割り当てた種別** `suggested_type_id`を併せて返す。既に共通登録済みでも、現在の共通種別と異なる種別を設定したユーザーがいる名前は**上書き候補**として `current_type_id`（現在の共通種別）付きで返す（同種別しか無い名前は返さない）
- 管理画面で候補を共通種別へ昇格するとき、**追加先の種別を選んでから昇格できる**（`POST /api/admin/excluded-items` に `exclusion_type_id` を渡す。未選択は既定種別「その他」）。**昇格した名前は `user_excluded_items` から削除される**（共通優先・重複排除）
- 管理画面「アイテム種別管理」の共通割当リストは、**種別ごとのタブ（「すべて」＋各種別。各タブに件数表示）で表示を切り替えられる**（フロント側の絞り込みのみ。`exclusion_type_id` が null の行は既定種別タブに含める）。全選択・一括削除は表示中タブ＋検索の絞り込み結果に対して働く

### dismissed_excluded_suggestions（共通化の候補から却下した名前・admin管理）
| カラム | 型 | 説明 |
|---|---|---|
| id | BIGINT PK | |
...
- `POST /api/items/match` — アイテム名（`names[]`）をまとめて登録済みアイテムと照合（完全一致＋末尾「...」「…」は前方一致）。アイテムボックスで使用
...
- `PUT  /api/listings/:id` — 編集（本人のみ）。`price` は**値下げのみ可能**（現在価格より高い値は 422。値上げするには取り下げて再出品）。価格を変更した場合は再出品時と同様に `bumped_at` を更新して「新着扱い」にする。**オークションは入札があると編集不可（400）**
- `DELETE /api/listings/:id` — 削除（本人/admin）。**オークションは入札があると取り下げ不可（400）**
- 一覧・詳細レスポンスはオークションのとき `current_price`（現在価格＝最良入札 or 開始価格）・`best_bid`・`bid_count` を付与する
- `POST /api/listings/:id/renew` — 出品期限を7日延長（`active` へ戻す）。再出品時は任意で `price`（1以上の整数）・`trade_type`（fixed/negotiable）を渡して価格・取引方法を変更できる（省略時は現状維持）。**値下げ**または**即決→交渉可**への変更があった場合は `bumped_at` を更新して「新着扱い」にする（新着順の先頭へ・宣伝ツイートの対象に含める）。**オークションは再出品不可（400）**
...
- `POST /api/chats/:id/deal` — 取引成立（取引対象を completed に変更し取引履歴を記録。交渉可は `final_price` で成立価格を指定可）。**オークションは自動成立のため手動 deal 不可（400）**
...
- `DELETE /api/buy-requests/:id` — 取り下げ（本人/admin）。**オークションは入札があると取り下げ不可（400）**
- `POST   /api/buy-requests/:id/renew` — 期限を1ヶ月延長（`active` へ戻す）。再登録時は任意で `price`（1以上の整数）・`trade_type`（fixed/negotiable）を渡して価格・取引方法を変更できる（省略時は現状維持）。**値下げ**または**即決→交渉可**への変更があった場合は `bumped_at` を更新して「新着扱い」にする。**オークションは再登録不可（400）**。`POST /api/buy-requests/:id/chats` はオークションでは入札（`bid_price` 必須）
...
- `PUT /api/mypage/inventory` — 台帳を全置換で保存（アカウント・カスタム種別はクライアントキー `key` で参照→サーバーがIDへ対応づけ。`accounts` / `items` / `exclusions` / `custom_types` を一括入れ替え）。`custom_types` はユーザーごとのカスタム種別 `[{key, name, sort_order}]`（後方互換のため省略可＝カスタム種別なし。**name 単位で upsert** し id を安定させ、payload に無いものは削除）。`exclusions` はユーザーの種別割当で、各要素は `{ name, exclusion_type_id, custom_type_key }`（後方互換のため文字列も許容＝`exclusion_type_id=null`。存在しない種別IDは null に丸め、`custom_type_key` が有効なら `user_exclusion_type_id` として保存＝共通種別より優先。無効なキーは共通種別の割当として扱う）。**本番（さくら）の WAF が実ボディ付き PUT を 403 で弾くため、フロントは `POST` + `X-HTTP-Method-Override: PUT` ヘッダで送る（ルートは PUT のまま。board.updatePost と同じホスト対策）**。サーバ登録対象外の行はクライアント側でペイロードから除外され、端末ローカルにのみ保存される（分割保存）
...
### お知らせ
- `GET  /api/announcements` — 表示中のお知らせ一覧（**認証不要**。Bearer トークンがあれば対象ユーザー `target_type` で絞り込み）
- `POST /api/announcements/:id/read` — 指定ユーザー向け（`specific`）お知らせを本人が既読化（要ログイン。本人を `target_user_ids` から外し、0人で削除。`all`/`staff` 向けや対象外ユーザーは 403）
...
### 項目名候補マスタ（付加効果/追加効果・editor / admin）
種別 `kind` は `bonus`（付加効果の項目名・default）/ `stat`（追加効果「その他」の項目名）。
- `GET    /api/admin/bonus-value-labels?kind=` — 一覧（種別ごと）
- `POST   /api/admin/bonus-value-labels` — 追加（body `{label, kind}`。同一種別内で重複不可）
- `POST   /api/admin/bonus-value-labels/organize` — 整理済み（左ペイン）の並びを**種別内で**確定。body `{ids:[...], kind}` の ids がそのまま整理済みの順序になり、同一種別で含まれない項目はすべて未整理へ戻す（未整理→整理済み・整理済み内の並べ替え・整理済み→未整理を1本で処理）。他種別の id は 422、他種別の整理状態には影響しない
- `POST   /api/admin/bonus-value-labels/:id/merge` — **統合**。未整理の項目名(:id)を同一種別の**整理済み**項目名（body `{target_id}`）へ統合する。統合元を使用しているアイテム側のデータを統合先の項目名へ一括更新（`kind='stat'`: `items.base_stats` の自由入力キーを付替え・アイテムが統合先キーを既に持つ場合は既存値を優先 / `kind='bonus'`: `item_bonus_effects.values[*].label` を付替え）し、統合元の項目名を削除する。表記ゆれ（誤字・別表記）の是正用。統合元が整理済み・統合先が未整理・種別違い・同一 id は 422。レスポンス `{ merged_into: {id, label}, updated_count }`
- `PUT    /api/admin/bonus-value-labels/:id` — 編集（種別は変更不可・同一種別内で重複不可）
- `DELETE /api/admin/bonus-value-labels/:id` — 削除
- （公開）`GET /api/bonus-value-labels?kind=` — 候補一覧（フォーム・絞り込み用。整理済み→未整理の順。`kind` 未指定は `bonus`）
...
### 除外アイテム（貼り付け除外）
- （公開）`GET /api/excluded-items` — 共通の種別割当と種別。`{ types: [{ id, name, is_default, default_enabled, sort_order }], items: [{ name, type_id }] }`（`type_id` は null のとき既定種別 id に正規化）。クライアントは `effectiveTypeId` で各行の表示種別を決める
...
- `GET    /api/admin/server-excluded-items` — サーバ登録対象外の全件（id 付き・admin）
- `POST   /api/admin/server-excluded-items` — 追加（admin。`names[]` で一括登録可・重複は無視）
- `DELETE /api/admin/server-excluded-items/:id` — 削除（admin）
- `DELETE /api/admin/server-excluded-items` — 一括削除（admin。`ids[]`）
- `GET    /api/admin/exclusion-types` — 種別の全件（admin。`sort_order`→`name` 順）
- `POST   /api/admin/exclusion-types` — 種別を追加（admin。`name` 必須・一意。`default_enabled` 任意＝省略時 true）
- `PUT    /api/admin/exclusion-types/:id` — 種別の更新（admin。`name`／`default_enabled` を部分更新。既定種別も改名・`default_enabled` 変更可・`is_default` は不変）
...
- **マイページの出品中／買取中の表示もバッチに依存せず判定する（多層防御）**: マイページ（`MyPage.tsx`）は `/mypage/listings`・`/mypage/buy-requests` が返す生の `status` をそのまま使わず、`status=active` でも `expires_at` が過去のものは「期限切れ」セクション（再出品導線）へ振り分ける。これにより本番 cron（1日1回）が走る前でも「残り−N日」の出品中カードが出ない。`completed` / `deal_failed` は期限に関わらず出品中側に残す。
...
- **マスキング**（`App\Support\ProdDataMasker`・マスキング後も判別可能な形を維持）:
  ...
  - ログイン情報: `users.password` を共通 dev パスワード（`password`）のハッシュへ、`users.email`（ブラインドインデックス）を `user{id}@dev.local` のローカルハッシュへ置換。これにより**どのユーザーにも `user{id}@dev.local` ＋ `password` でローカルログイン可能**（`role`/`id` は保持）。`remember_token` は破棄。
  - 取込結果の要約に admin/editor のローカルログイン用メール一覧を出力する。
- **起動**: ヘッダー「管理」メニューの「**本番データ取込**」ボタン（**ローカル環境・admin 限定で表示**、`import.meta.env.DEV`）から `POST /api/admin/dev/pull-prod`（ローカル専用・本番は 403）を呼ぶ。確認ダイアログ→実行→要約をダイアログ表示。確認ダイアログには事前に起動すべき**SSHトンネルの起動コマンドをコピー可能な等幅ブロックで表示**する（`DialogContext` の `code` オプション）。実コマンド（本番ホスト名・SSHユーザー）は公開リポジトリに置かないため、gitignore 済み `frontend/.env.local` の `VITE_PROD_TUNNEL_CMD` から読み、未設定時はプレースホルダ入りの汎用コマンドを表示する。チャット本文や掲示板投稿などの自由記述に含まれうる個人情報はマスキング対象外（構造化された本番固有情報のみマスクする）。

---

## 利用規約・プライバシーポリシー同意フロー

新規登録画面（`/auth/register`）を開くと、利用規約モーダルを画面前面に表示し、同意するまで登録手続きへ進めない。同意の対象は利用規約およびプライバシーポリシーの両方。

### 動作仕様
- 登録画面マウント時にモーダル（`TermsModal`）を自動表示。`agreed` 状態が `false` の間は常に前面に表示し、背後の登録フォームは操作できない。
- モーダルには利用規約の全文をスクロール領域で表示。ヘッダーと第7条からプライバシーポリシー（`/privacy`・別タブ）へリンクする。
- 「同意する」を押下すると `agreed = true` となりモーダルを閉じ、登録フォームが操作可能になる。
- 「同意しない」を押下するとトップページ（`/`）へ遷移する。
- 同意前は「登録する」ボタンを無効化（`disabled`）。さらに送信処理（`handleSubmit`）の冒頭でも同意チェックを行い、未同意の場合は「利用規約およびプライバシーポリシーに同意してください」を表示して処理を中断する（二重の安全策）。
...
5. 第5条（免責事項）
6. 第6条（規約の変更）
7. 第7条（個人情報の取扱い） — プライバシーポリシーに従う旨とリンク

※ 文面は暫定。正式リリース前に確定版へ差し替える。

### 実装
- 規約本文: `frontend/src/components/TermsContent.tsx`（第1〜7条。モーダルと常設ページで共用）
- コンポーネント: `frontend/src/components/TermsModal.tsx`（`onAgree` / `onDecline` を props で受け取る）
- 常設ページ: `frontend/src/pages/TermsPage.tsx`（`/terms`・公開。フッターからリンク）
...
個人情報の保護に関する法律（個人情報保護法）が求める公表事項を、公開ページ `/privacy`（`frontend/src/pages/PrivacyPolicyPage.tsx`）で公表する。フッター（`Footer.tsx`）の「利用規約」「プライバシーポリシー」リンクから未ログインでも常時閲覧できる。
...
8. 開示・訂正・利用停止・削除等の請求（退会を含む） — 窓口で受け付け、本人確認のうえ法令に従い対応。他ユーザーの記録に影響する投稿は匿名化（「退会ユーザー」表示への置換等）で対応する場合がある
9. お問い合わせ窓口 — サイト内掲示板「お問い合わせ」
10. 改定手続き

### 実装メモ
- 退会（アカウント削除）のセルフサービス機能は未実装。現状は窓口（掲示板）経由の請求ベースで対応する（ポリシー第8章に明記）。セルフ退会機能は将来課題。
- 外部のアクセス解析・広告を導入する場合は、第7章（Cookie等）の記載を必ず更新すること。
...
| `tests/Feature/AdminAnalyticsApiTest.php` | 利用状況解析API（権限401/403（一般）/editor許可・日次集計・登録の合算 `registrations`・JST日付境界・期間外除外・`is_valid=false` 除外・出品由来/買取由来の分割・取引ユニークユーザー数・日次ユニークアクセス `active_users`（期間ユニーク合計・同日複数時間帯でも1人・ゼロ埋め）・アクセス記録ミドルウェア（JST日付＋時で記録・同じ時間帯は重複なし・時間帯が変われば別行・未認証は記録しない）・時間帯分布 `hourly`（日付をまたいだ同一時刻の合算・JST時刻変換・0〜23時ゼロ埋め・期間合計との一致・アクセスは時間帯ごとのユニークユーザーの延べ数で `hour` が null の古い行は除外）・`days` バリデーション・既定30日） |
...
| `src/pages/NotificationSettingsPage.test.tsx` | 通知設定画面（種別×チャネルのトグル表示・変更即保存・保存失敗で元に戻す・宛先未設定の案内・別アドレスは確認メール案内・確認済み表示と削除）＋**マイページから移設したブラウザ通知の操作**（ON時の配信中表示と「OFFにする」・OFF中の「ONに戻す」・未許可の許可要求・ブロック/非対応の案内ダイアログ）＋**画面の並びとPWA再導線**（ブラウザ通知セクションを種別より先頭に表示・インストール可能かつ「あとで」済みなら先頭に「アプリをインストール」／バナー表示中・インストール不可・standalone では出さない） |
...
| `src/components/PriceAnalytics.test.tsx` | 価格解析（0埋め時の「—」表示・「相場対象外」「他サイト」バッジ・売り/買い相場タブ切替・Xで検索リンクと検索語整形） |
...
| `src/pages/OwnedItemsPage.test.tsx` | アイテムボックス（自動再紐づけ・表示切替/種別タブ・重複確認・種別変更・カスタム種別・公式DBリンク・貼り付けアコーディオン・買取ありフィルタ・紐づけ済み行のアイテム名クリックで詳細モーダル表示・レスポンシブ表示（lg未満のカード型リストに全操作が揃う/アカウント・種別切替のドロップダウン化と絞り込み/編集の保存/空一覧の案内/lg以上はテーブル/lg以上でもアカウントタブが上限超ならドロップダウン化・上限以内はピル型タブのまま）） |
...
| `src/pages/admin/AdminAnalyticsPage.test.tsx` | 利用状況解析ページ（初期表示は「登録」「成立」「アクセス」3系列のみ・サマリーカード（アクセスは「人」表記）・系列チェックボックスの表示/非表示切替・「すべて」の一括切替と全非表示時の案内文・時間帯分布の棒グラフ（チェックボックス共用・アクセスも含む7系列）） |
...
- [x] お問い合わせ掲示板（スレッド・投稿・admin管理）
- [x] ミスリルフラグ
- [x] 本番サーバーへのデプロイ設定（手順書・スクリプト一式は非公開運用ドキュメントで管理・初回リリース）
...
- [x] フロントエンド単体テスト（Vitest + Testing Library・jsdom。種別判定／装備セット集約／マスタ定数整合／アイテムボックスの表示種別・分割保存／利用規約同意フロー／価格解析表示／出品一覧のタブ・絞り込み）

### Phase 5: レスポンシブ対応
- [ ] レスポンシブデザイン対応（スマートフォン・タブレット向けレイアウト）

### 追加実装（運用フェーズ）
- [x] アイテムボックス（公式の所持アイテム一覧を貼り付けて台帳管理・表示種別／サーバ登録対象外・`POST /api/items/match`）
...
デプロイ手順書・自動化スクリプト・cron 登録・本番 `.env` テンプレート・SSHトンネルなどの運用一式は、
本番サーバーの実情報（アカウント名・ホスト名・サーバー上のパス等）を含むため、
**リポジトリ外の非公開運用ドキュメント（`OPERATIONS.md`）と `deploy/` スクリプト群で管理する**（Git 追跡しない・`.gitignore` 済み）。
MoE公式素材の画像（公式バナー・ファンサイトキット由来。「レイアウト・ブランド表示」参照）も同様に非公開管理とする。

### リポジトリ公開（ポートフォリオ）ポリシー
- 本リポジトリはポートフォリオとして公開する。ルートに `README.md`（プロジェクト概要・非公開管理の注記・MoE著作権表記）と `LICENSE`（閲覧用・無断複製/再配布/商用利用不可）を置く
- 公開リポジトリに**含めない**もの: 本番実情報（deploy/ 一式・運用手順の実値）・MoE公式素材の画像・`.env` 実体
- コミット作者メールは GitHub の noreply アドレスを使う（リポジトリ local 設定済み）

公開リポジトリに残す運用情報は以下のレベルに留める:

- **ホスティング**: レンタルサーバー（共用）。root・Docker・nginx 不可のため **Apache + .htaccess** で `/api` と SPA を振り分け、公開URL は `https://moe-trade.sakuraweb.com`
- **React**: サーバに Node が無いため手元PCでビルドし、`dist` の中身を `backend/public/` にアップロード
- **キュー**: 常駐プロセス不可のため `QUEUE_CONNECTION=sync`（即時実行）
- **認証**: Bearerトークン方式・API は相対パス `/api` → 同一ドメインで無調整で動作
- **SSL**: Let's Encrypt 無料SSL ＋ HTTPS転送ON
- **定期バッチ**: `routes/console.php` のスケジュール定義が正。本番はコマンド直叩きの cron ラッパー（非公開）で実行
- **本番 `.env` の重要キー**: `APP_DEBUG=false` / `APP_ENV=production` / `FRONTEND_URL`（メール認証後のリダイレクト先）/ `EMAIL_HASH_KEY`（メールハッシュのペッパー。**一度決めたら変更しない**）
