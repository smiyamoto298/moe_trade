...
| bumped_at | TIMESTAMP NULL | 「新着扱い」時刻。期限切れの再出品で**値下げ**または**即決→交渉可**にしたとき `now()` を記録。新着順の並び替えと宣伝ツイートの対象選定は `COALESCE(bumped_at, created_at)` を基準にする（未設定の通常出品は `created_at` と同じ挙動） |
| created_at / updated_at | TIMESTAMP | |
...
| bumped_at | TIMESTAMP NULL | 「新着扱い」時刻。`listings.bumped_at` と同様、値下げ／即決→交渉可での再登録時に記録し新着順・宣伝の基準に使う |
| created_at / updated_at | TIMESTAMP | |
