...
- **共通除外の種別（カテゴリ）と適用選択**: 共通除外は種別（`exclusion_types`・既定「その他」）で分類される。ユーザーは個別除外リストとは**別の「共通除外の設定」ボタン／モーダル**で適用範囲を選べる（**端末ローカル保存**・未設定は全適用＝オプトアウト方式。マイペ整理と一括出品で共通）。
...
- ユーザーはマイペ整理で「適用する種別」を選べる（端末ローカル保存・既定は全種別を適用＝オプトアウト方式）。[フロント参照: `selectedCommonNames` / `getAppliedExclusionTypeIds`]
