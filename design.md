...
- **ヘッダーのナビ構成**: トップ階層に `出品一覧` / `買取一覧` / `アイテム一覧`（`/items`・全ユーザー。`unverifiedItemCount` のバッジ付き）を並べ、ログイン時はその後ろに `マイ取引`（`/mypage`）/ `アイテムボックス`（`/mypage/items`）/ `運営掲示板`（管理メニューの直前）を表示する。マイページ階層・アイテム一覧は以前ドロップダウン配下だったがトップ階層へ移動済み
...
登録・編集フォームで「その他」配下を選ぶと「適切な種別がない場合、運営掲示板でご連絡おねがいします！／時の石など、Ancient Age で取引可能な種別は対象外とさせていただいています！」の案内を表示する。
