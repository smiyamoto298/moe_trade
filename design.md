...
- **オークションは専用バッチ `auctions:resolve` を15分ごとに実行**（`routes/console.php` で `everyFifteenMinutes()` 登録）：期限日到来時に open 入札があれば最良入札（出品=最高／買取=最安）を自動成立（取引履歴を記録）、入札が無ければ `expired`（再出品はしない）。共通成立ロジックは `App\Support\Auction`。本番でコマンド直叩きの環境は `deploy/cron-resolve-auctions.sh` を15分ごとに cron 登録
...
/home/<アカウント名>/www/moe_trade/deploy/cron-expire-listings.sh    # 1日1回（例 4:00）
/home/<アカウント名>/www/moe_trade/deploy/cron-resolve-auctions.sh   # 15分ごと（オークション解決）
```
...
- **接続経路（重要）**: さくら共用サーバは**外部からの MySQL(3306) をファイアウォールで遮断**しているため、直結はできない（タイムアウト）。**SSHトンネル経由**で接続する: ホスト側で `bash deploy/tunnel-prod-db.sh`（または `deploy/tunnel-prod-db.ps1`）を起動し `localhost:13306 → mysqlXXXX….db…:3306` を中継、`.env` は `PROD_DB_HOST=host.docker.internal` / `PROD_DB_PORT=13306` を指す（php コンテナ→ホストのトンネル出口→さくら内部→本番DB）。さくらは SSH の TCP転送を許可している。取込前にトンネルを起動しておくこと。
...
| `cron-resolve-auctions.sh` | オークション自動成立／取り下げバッチ（`auctions:resolve`）の cron 用ラッパー（15分ごと） |
| `cron-purge-announcements.sh` | 期限切れお知らせ削除バッチ（`announcements:purge-expired`）の cron 用ラッパー（毎日6:00） |
| `tunnel-prod-db.sh` / `.ps1` | 本番データのローカル取込用 SSHトンネル（`localhost:13306 → 本番MySQL:3306`）。取込前にホスト側で起動する |
