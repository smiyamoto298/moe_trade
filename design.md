...
- アイテム登録フォームの項目名候補入力（`ComboInput` の自前ドロップダウン。IME 変換中の連結不具合を避けるためネイティブ datalist は不使用）・一覧の絞り込み候補に使用。アイテム登録時に未登録ラベルは自動追加（`BonusValueLabel::syncFromBonusEffects()`、自動追加分は **未整理** で入る）
