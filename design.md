...
- **一覧は出品一覧と同様のテーブルレイアウト**（アイテム／取引／価格／操作列。`listing-table-container` のコンテナクエリで狭い幅では取引列を畳む）。買取一覧では装備性能・確認ステータスは表示せず、アイテム名（種別・公式DBリンク）・取引条件・価格のみを表示する
...
- **テキスト**: 値欄が数値入力からテキスト入力に変わり、文字列をそのまま保持・表示する。
  ...
  テキスト値が **5文字以上**（`TEXT_DETAIL_THRESHOLD`）の場合は値を直接展開せず「詳細」バッジ（専用技バッジと同デザインの青系）で表示し、ホバー（フォーカス）で全文をポップアップ表示する（長文でレイアウトが崩れるのを防ぐ）。4文字以下はそのまま表示。
