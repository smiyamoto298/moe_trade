...
- `POST /api/items` — 登録（全ログインユーザー。unverified で作成。`skill_requirements`、アセット項目 `placement` / `asset_width` / `asset_height` / `storage_count` / `special_function`、その他項目 `pet_name` / `recipe_name` / `recipe_binder` 対応。新規バインダー名は `binder_labels` へ自動追加。`fixed_hashtags`（固定タグ）は editor/admin のみ反映、`user_hashtags`（通常タグ）はログインユーザーなら反映）
